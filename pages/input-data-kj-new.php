	...
	<div class="box box-info">
		...
		<div class="box-body">
			...
			<div class="col-md-6">
					...
					<div class="col-sm-3">
						<div class="input-group">
							<input name="sisa" type="text" class="form-control" id="sisa" value="<?php if ($cek > 0) {
																																											echo number_format($rcek['sisa'], 2);
																																										} else {
					...
					<div class="col-sm-4">
						<div class="input-group date">
							...
							<input name="tglmsk" type="text" class="form-control pull-right" id="datepicker3" placeholder="0000-00-00" value="<?php if ($cek > 0) {
																																																																	echo ($rcek['tgl_masuk'] ? date_format($rcek['tgl_masuk'], 'Y-m-d') : '');
																																																																} ?>" required />
						</div>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-8">
						<textarea name="cekwarna" class="form-control" id="cekwarna" placeholder="Cek Warna"><?php if ($cek > 0) {
																																																		...
																																																		echo $sWarna['status'] . " " . (!empty($sWarna['tgl_update']) ? date_format($sWarna['tgl_update'], 'Y-m-d') : '');
																																																	} ?></textarea>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-3">
						<div class="input-group">
							<input name="extra" type="text" class="form-control" id="extra" value="<?php if ($cek > 0) {
																																												echo number_format($rcek['berat_extra'], 2);
																																											} ?>" placeholder="0.00" style="text-align: right;">
							<span class="input-group-addon">KG</span>
						</div>
					</div>
					<div class="col-sm-4">
						<div class="input-group">
							<input name="extra_p" type="text" class="form-control" id="extra_p" value="<?php if ($cek > 0) {
																																														echo number_format($rcek['panjang_extra'], 2);
																																													} ?>" placeholder="0.00" style="text-align: right;" required>
							<span class="input-group-addon">
								<select name="satuan1" style="font-size: 12px;">
					...
					<div class="col-sm-3">
						<div class="input-group">
							<input name="estimasi" type="text" class="form-control" id="estimasi" value="<?php if ($cek > 0) {
																																															echo number_format($rcek['estimasi'], 2);
																																														} ?>" placeholder="0.00" style="text-align: right;">
							<span class="input-group-addon">KG</span>
						</div>
					</div>
					<div class="col-sm-4">
						<div class="input-group">
							<input name="estimasi_p" type="text" class="form-control" id="estimasi_p" value="<?php if ($cek > 0) {
																																																	echo number_format($rcek['panjang_estimasi'], 2);
																																																} ?>" placeholder="0.00" style="text-align: right;" required>
							<span class="input-group-addon">
				...
				<input type="hidden" value="<?php if ($cek > 0) {
																			echo (!empty($rcek['tgl_delivery']) ? date_format($rcek['tgl_delivery'], 'Y-m-d') : '');
																		} else {
					...
					<div class="form-group">
						<textarea id="editor1" name="editor1" rows="10" cols="80" class="form-control"><p>Dear mkt team,</p>
							...
							<tr>
								...
								<td width="723" nowrap valign="bottom"><p><?php echo (!empty($rcek['tgl_delivery']) ? date_format($rcek['tgl_delivery'], 'Y-m-d') : ''); ?></p></td>
							</tr>
							<tr>
...
function nobon()
{
	...
	$format = date("y");
	$sql = sqlsrv_query($con_db_qc_sqlsrv, "SELECT TOP 1 bpp FROM db_qc.tbl_qcf WHERE YEAR(tgl_masuk)=YEAR(GETDATE()) ORDER BY bpp DESC") or die(print_r(sqlsrv_errors(), true));
...
if ($_POST['save'] == "save") {
	...
	$rol_mslh = intval($_POST['rol_mslh']);
	$lot = trim($_POST['lot']);
	...
	// normalize numeric inputs to avoid SQL Server conversion errors
	$qty1 = floatval(str_replace(',', '.', $_POST['qty1']));
	$qty2 = floatval(str_replace(',', '.', $_POST['qty2']));
	$qty3 = intval($_POST['qty3']);
	$qty4 = floatval(str_replace(',', '.', $_POST['qty4']));
	$extra = floatval(str_replace(',', '.', $_POST['extra']));
	$extra_p = floatval(str_replace(',', '.', $_POST['extra_p']));
	$estimasi = floatval(str_replace(',', '.', $_POST['estimasi']));
	$estimasi_p = floatval(str_replace(',', '.', $_POST['estimasi_p']));
	...
	$sqlData = sqlsrv_query($con_db_qc_sqlsrv, "INSERT INTO db_qc.tbl_qcf 
			(bpp, nokk, nodemand, pelanggan, no_order, no_hanger, no_item, no_po, no_ko, jenis_kain, styl, 
			 berat_order, panjang_order, satuan_order, rol_bruto, berat_bruto, lebar, gramasi, berat_extra, panjang_extra, estimasi, 
			 panjang_estimasi, lot, rol, warna, no_warna, cek_warna, netto, panjang, satuan, sisa, tgl_masuk, tgl_delivery, qty_mslh, 
			 rol_mslh, t_jawab, persen, sts_pbon, sts_nodelay, sts_tembakdok, masalah, ket, sales, tgl_update)
			VALUES ('$nou', '$_POST[nokk]', '$_POST[nodemand]', '$_POST[pelanggan]', '$_POST[no_order]', '$_POST[no_hanger]', '$_POST[no_item]', '$po', '$_POST[no_ko]', '$jns', '$styl',
			 $qty1, $qty2, '$_POST[satuan1]', $qty3, $qty4, $lebar, $grms, $extra, $extra_p, $estimasi,
			 $estimasi_p, '$lot', $rol, '$warna', '$nowarna', '$cekwarna', $netto, $panjang, '$_POST[satuan2]', $sisa, '$_POST[tglmsk]', '$_POST[tgl_delivery]', $qty_mslh,
			 $rol_mslh, '$multijawab', '$persen', '$sts_pbon', '$sts_nodelay', '$sts_tembakdok', '$masalah', '$ket1', '$sales', GETDATE())");
...
if ($_POST['update'] == "update") {
	...
	$rol = intval($_POST['rol']);
	$rol_mslh = intval($_POST['rol_mslh']);
	$rol_bruto = intval($_POST['rol_bruto']);
	$lot = trim($_POST['lot']);
	$t_jawab = $_POST['t_jawab'];
	$multijawab = "";
	$persen = $_POST['persen'];
	...
	$sqlData = sqlsrv_query($con_db_qc_sqlsrv, "UPDATE db_qc.tbl_qcf SET 
			...
			gramasi=$grms,
			berat_extra=$extra,
			panjang_extra=$extra_p,
			estimasi=$estimasi,
			panjang_estimasi=$estimasi_p,
			lot='$lot',
			rol=$rol,
			warna='$warna',
			no_warna='$nowarna',
