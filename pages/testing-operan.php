...
if (isset($_POST['no_kkold'])) {
	$Nokkold = $_POST['no_kkold'] ; 
} else if (isset($_POST['fl_nodemand'])) {
	$Nokkold = $_POST['kk_nodemand'] ; 
} else {
	$Nokkold = '';
}
$sqlCek=sqlsrv_query($con_db_qc_sqlsrv,"SELECT TOP 1 * FROM db_qc.tbl_tq_nokk WHERE (nodemand='$nodemand' OR nodemand='$nodemand_post') ORDER BY id DESC");
                  ...
                  <label for="no_po" class="col-sm-3 control-label">No Demand</label>
                  	<div class="col-sm-4">
						<div class="input-group">	  
							...
							onchange="window.location='TestingOperan-'+this.value" value="<?php if($_GET['nodemand']!=""){echo $_GET['nodemand'];}else{echo $nodemand_post;}?>" placeholder="No Demand" required <?php if($_SESSION['lvl_id']=="TQ"){echo "readonly";}?>>
						</div>	  
		  			</div>
            </div>
			<div class="form-group">
...
<?php if($_GET['nodemand']!="" OR $_GET['notest']!=""){?>
<div class="box box-success">
    ...
    <div class="box-body">
        <div class="col-md-6">
            <div class="form-group">
				<label for="no_kkold" class="col-sm-3 control-label">No Demand</label>
				<div class="col-sm-4">
                    <input name="no_kkold" type="text" class="form-control" id="no_kkold" placeholder="No Demand" autocomplete="off" 
...
$cekold   = sqlsrv_has_rows($sqlCekold) ? 1 : 0;
$rcekold  = sqlsrv_fetch_array($sqlCekold, SQLSRV_FETCH_ASSOC);

$sqlCekNew = sqlsrv_query(
    $con_db_qc_sqlsrv,
    "SELECT TOP 1 *,
        CONCAT_WS(' ',
            fc_note, ph_note, abr_note, bas_note, dry_note, fla_note,
            fwe_note, fwi_note, burs_note, repp_note, wick_note,
            absor_note, apper_note, fiber_note, pillb_note, pillm_note,
            pillr_note, thick_note, growth_note, recover_note,
            stretch_note, sns_note, snab_note, snam_note, snap_note,
            wash_note, water_note, acid_note, alkaline_note, crock_note,
            phenolic_note, cm_printing_note, cm_dye_note, light_note,
            light_pers_note, saliva_note, h_shrinkage_note, fibre_note,
            pilll_note, soil_note, apperss_note, bleeding_note,
            chlorin_note, dye_tf_note, humidity_note, odour_note,
            curling_note
        ) AS note_g
...
$cekNew  = sqlsrv_has_rows($sqlCekNew) ? 1 : 0;
$rcekNew = sqlsrv_fetch_array($sqlCekNew, SQLSRV_FETCH_ASSOC);
...
$sqlCek1 = sqlsrv_query(
    ...
    "SELECT TOP 1 *,
        CONCAT_WS(' ',
            fc_note, ph_note, abr_note, bas_note, dry_note, fla_note,
            fwe_note, fwi_note, burs_note, repp_note, wick_note, wick_note,
            absor_note, apper_note, fiber_note, pillb_note, pillm_note,
            pillr_note, thick_note, growth_note, recover_note,
            stretch_note, sns_note, snab_note, snam_note, snap_note,
            wash_note, water_note, acid_note, alkaline_note, crock_note,
...
$sqlCekR = sqlsrv_query(
    ...
    "SELECT *,
        CONCAT_WS(' ',
            rfc_note, rph_note, rabr_note, rbas_note, rdry_note,
            rfla_note, rfwe_note, rfwi_note, rburs_note, rrepp_note,
            rwick_note, rabsor_note, rapper_note, rfiber_note,
            rpillb_note, rpillm_note, rpillr_note, rthick_note,
            rgrowth_note, rrecover_note, rstretch_note, rsns_note,
            rsnab_note, rsnam_note, rsnap_note, rwash_note, rwater_note,
            racid_note, ralkaline_note, rcrock_note, rphenolic_note,
            rcm_printing_note, rcm_dye_note, rlight_note,
            rlight_pers_note, rsaliva_note, rh_shrinkage_note,
            rfibre_note, rpilll_note, rsoil_note, rapperss_note,
            rbleeding_note, rchlorin_note, rdye_tf_note,
            rhumidity_note, rodour_note, rcurling_note
        ) AS rnote_g
     FROM db_qc.tbl_tq_randomtest
     WHERE no_item = ? OR no_hanger = ?",
    [$rcekold['no_item'], $rcekold['no_hanger']]
);
$cekR  = sqlsrv_has_rows($sqlCekR) ? 1 : 0;
...
$sqlCekD = sqlsrv_query(
    ...
    "SELECT TOP 1 *,
        CONCAT_WS(' ',
            dfc_note, dph_note, dabr_note, dbas_note, ddry_note,
            dfla_note, dfwe_note, dfwi_note, dburs_note, drepp_note,
            dwick_note, dabsor_note, dapper_note, dfiber_note,
            dpillb_note, dpillm_note, dpillr_note, dthick_note,
            dgrowth_note, drecover_note, dstretch_note, dsns_note,
            dsnab_note, dsnam_note, dsnap_note, dwash_note, dwater_note,
            dacid_note, dalkaline_note, dcrock_note, dphenolic_note,
            dcm_printing_note, dcm_dye_note, dlight_note,
            dlight_pers_note, dsaliva_note, dh_shrinkage_note,
            dfibre_note, dpilll_note, dsoil_note, dapperss_note,
            dbleeding_note, dchlorin_note, ddye_tf_note,
            dhumidity_note, dodour_note, dcurling_note
        ) AS dnote_g
...
$cekD  = sqlsrv_has_rows($sqlCekD) ? 1 : 0;
...
$rcekcmt = sqlsrv_fetch_array($sqlCmt, SQLSRV_FETCH_ASSOC);

//penambahan 
$result_sql = sqlsrv_query(
    $con_db_qc_sqlsrv,
    "SELECT TOP 1 a.id
...
$result_count = sqlsrv_has_rows($result_sql) ? 1 : 0;
?>
<?php if($cekold>0){ ?>
<div class="box box-success">
   	<div class="box-header with-border">
    	<h3 class="box-title">Detail Testing1</h3>
		<div class="box-tools pull-right">
			
			<?php 
				$fl_sql  = sqlsrv_query($con_db_qc_sqlsrv,"SELECT TOP 1 id FROM db_qc.tbl_tq_first_lot WHERE nodemand = ? ORDER BY id DESC", [$nodemand]);
				$fl_data = sqlsrv_fetch_array($fl_sql, SQLSRV_FETCH_ASSOC);
				if ($fl_data) { ?>	
					<!--
					<form action="" method="POST">
						<input type="hidden" name="fl_nodemand" value="<?=$nodemand?>">
						<input type="hidden" name="kk_nodemand" value="<?=$Nokkold?>">
						<button type="submit" name="update_first_lot" class="btn btn-default" >Update First Lot <?=$nodemand?> </button>
					</form>
					-->
					...
					<form class="form-horizontal" action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
						<div class="form-group">
          					...
          					<div class="col-sm-4">
								...
								<?php
								$sql = "SELECT a.*, b.* From db_qc.tbl_tq_nokk a INNER JOIN db_qc.tbl_master_test b ON a.no_test=b.no_testmaster WHERE nodemand='$Nokkold'";
						...
						<div class="form-group" id="fc7" style="display:none;">
							...
							<div class="col-sm-1">
								...
								$tq_test_2_sql	 = sqlsrv_query($con_db_qc_sqlsrv,"SELECT id_nokk, spirality_status from db_qc.tbl_tq_test_2 where id_nokk = '$id_tq_test_2'");
								$tq_test_2_array = sqlsrv_fetch_array($tq_test_2_sql);

								$spirality_status = ['DISPOSISI','A','R','PASS','FAIL','RANDOM']; ?>
									
									<input type="hidden" name="id_nokk_new" value=<?=$rcek['id']?>>
									
									<select name="spirality_status"  class="form-control select2" id="stat_sparility" onChange="tampil();" >
						...
						<div class="form-group" id="dis_spirality_status" style="display:none;">
									<div class="col-sm-5"> 
										...
										<div class="col-sm-1"><input name="dspirality1" value="<?php echo $rcekD['dspirality1'];?>" type="text" class="form-control"></div>
										<div class="col-sm-1"><input name="dspirality2" value="<?php echo $rcekD['dspirality2'];?>" type="text" class="form-control"></div>
										<div class="col-sm-1"><input name="dspirality3" value="<?php echo $rcekD['dspirality3'];?>" type="text" class="form-control"></div>
										<div class="col-sm-1"><input name="dspirality4" value="<?php echo $rcekD['dspirality4'];?>" type="text" class="form-control"></div>
										<div class="col-sm-1"><input name="dspirality5" value="<?php echo $rcekD['dspirality5'];?>" type="text" class="form-control"></div>
										<div class="col-sm-1"><input name="dspirality6" value="<?php echo $rcekD['dspirality6'];?>" type="text" class="form-control"></div>
						...
						<div class="form-group" id="disff" style="display:none;">
							...
							<!-- CURLING END-->
						<div class="form-group">
							<?php if($nodemand!="" OR $nodemand_post!=""){ ?>
...
<?php if($cek1>0 and $result_count > 0 ){ ?>
      ...
      <div class="row">
        <div class="col-xs-12">
          <h2 class="page-header">
            ...
            <small class="pull-right">Date: <?php echo ($rcek1['tgl_buat'] ? date_format($rcek1['tgl_buat'], "Y-m-d"): '');?></small>
          </h2>
        </div>
        <!-- /.col -->
      </div>
...
if($_POST['physical_save']=="save") {
	...
	$tq_test_2_sql	 = sqlsrv_query($con_db_qc_sqlsrv,"SELECT id_nokk, spirality_status from db_qc.tbl_tq_test_2 where id_nokk = '$id_nokk_new' ");
	$tq_test_2_array = sqlsrv_fetch_array($tq_test_2_sql);
	
	if ($tq_test_2_array) {
		if ($_POST['spirality_status']=='0') { //update
			$second = '/deleted'.date('is');
			$sqlPHY=sqlsrv_query($con_db_qc_sqlsrv,"UPDATE db_qc.tbl_tq_test_2 SET id_nokk = concat('$id_nokk_new','$second') WHERE id_nokk='$id_nokk_new'");
		}else  {
			$sqlPHY=sqlsrv_query($con_db_qc_sqlsrv,"UPDATE db_qc.tbl_tq_test_2 SET spirality_status='$spirality_status' WHERE id_nokk='$id_nokk_new'");
		}	
	} else {
		if ($_POST['spirality_status']!='0') { //insert 
		$sql_no_demand =sqlsrv_query($con_db_qc_sqlsrv,"INSERT INTO db_qc.tbl_tq_test_2 (id_nokk,spirality_status) VALUES ('$id_nokk_new','$spirality_status')");				
		}
	}
}
?>
...
if($_POST['physical_save']=="save" AND $cekNew>0){
	...
	if($sqlPHY){
		...
		echo "<script>swal({
			...
			if (result.value) {
				...
				window.location.href='TestingOperan-$nodemand'; 
			}
		});</script>";
	}
	...
	if($sqlPHY){
		...
		$sqlPHYDI=sqlsrv_query($con_db_qc_sqlsrv,"INSERT INTO db_qc.tbl_tq_disptest 
			(
				id_nokk, dflamability, dfla_note, dfc_cott, dfc_poly, dfc_elastane, dfc_cott1, dfc_poly1, dfc_elastane1, 
				std_dfc_cott1, std_dfc_poly1, std_dfc_elastane1, dfibercontent, dfiber_note, dfc_wpi, dfc_cpi, dfc_note, 
				df_weight, dfwe_note, df_width, dfwi_note, dbow, dskew, dbas_note, dh_shrinkage_temp, dh_shrinkage_l1, 
				dh_shrinkage_w1, dh_shrinkage_grd, dh_shrinkage_app, dh_shrinkage_note, dss_temp, dss_washes3, dss_washes10, 
				dss_washes15, dss_cmt, dshrinkage_l1, dshrinkage_l2, dshrinkage_l3, dshrinkage_l4, dshrinkage_l5, dshrinkage_l6, 
				dshrinkage_w1, dshrinkage_w2, dshrinkage_w3, dshrinkage_w4, dshrinkage_w5, dshrinkage_w6, dspirality1, 
				dspirality2, dspirality3, dspirality4, dspirality5, dspirality6, dss_linedry, dss_tumbledry, dsns_note, 
				dapperss_ch1, dapperss_ch2, dapperss_ch3, dapperss_ch4, dapperss_cc1, dapperss_cc2, dapperss_cc3, dapperss_cc4, 
				dapperss_st, dapperss_pf1, dapperss_pf2, dapperss_pf3, dapperss_pf4, dapperss_pb1, dapperss_pb2, dapperss_pb3, 
				dapperss_pb4, dapperss_sf1, dapperss_sf2, dapperss_sf3, dapperss_sf4, dapperss_sb1, dapperss_sb2, dapperss_sb3, 
				dapperss_sb4, dapperss_note, dpm_f1, dpm_b1, dpm_f2, dpm_b2, dpm_f3, dpm_b3, dpm_f4, dpm_b4, dpm_f5, dpm_b5, 
				dpillm_note, dpl_f1, dpl_b1, dpl_f2, dpl_b2, dpl_f3, dpl_b3, dpl_f4, dpl_b4, dpl_f5, dpl_b5, dpilll_note, 
				dpb_f1, dpb_b1, dpb_f2, dpb_b2, dpb_f3, dpb_b3, dpb_f4, dpb_b4, dpb_f5, dpb_b5, dpillb_note, dprt_f1, dprt_b1, 
				dprt_f2, dprt_b2, dprt_f3, dprt_b3, dprt_f4, dprt_b4, dprt_f5, dprt_b5, dpillr_note, dabration, dabr_note, 
				dsm_l1, dsm_w1, dsm_l2, dsm_w2, dsm_l3, dsm_w3, dsm_l4, dsm_w4, dsnam_note, dsp_grdl1, dsp_clsl1, dsp_shol1, 
				dsp_medl1, dsp_lonl1, dsp_grdw1, dsp_clsw1, dsp_show1, dsp_medw1, dsp_lonw1, dsp_grdl2, dsp_clsl2, dsp_shol2, 
				dsp_medl2, dsp_lonl2, dsp_grdw2, dsp_clsw2, dsp_show2, dsp_medw2, dsp_lonw2, dsnap_note, dsb_l1, dsb_w1, dsb_l2, 
				dsb_w2, dsb_l3, dsb_w3, dsb_l4, dsb_w4, dsnab_note, dbs_instron, dbs_mullen, dbs_tru, dburs_note, dthick1, 
				dthick2, dthick3, dthickav, dthick_note, dstretch_l1, dstretch_w1, dstretch_l2, dstretch_w2, dstretch_l3, 
				dstretch_w3, dstretch_l4, dstretch_w4, dstretch_l5, dstretch_w5, dload_stretch, dstretch_note, drecover_l1, 
				drecover_w1, drecover_l2, drecover_w2, drecover_l3, drecover_w3, drecover_l4, drecover_w4, drecover_l5, 
				drecover_w5, drecover_note, dgrowth_l1, dgrowth_w1, dgrowth_l2, dgrowth_w2, drec_growth_l1, drec_growth_w1, 
				drec_growth_l2, drec_growth_w2, dgrowth_note, dapper_ch1, dapper_ch2, dapper_ch3, dapper_cc1, dapper_cc2, 
				dapper_cc3, dapper_st, dapper_st2, dapper_st3, dapper_pf1, dapper_pf2, dapper_pf3, dapper_pb1, dapper_pb2, 
				dapper_pb3, dapper_acetate, dapper_cotton, dapper_nylon, dapper_poly, dapper_acrylic, dapper_wool, dapper_note, 
				dfibre_transfer, dfibre_grade, dfibre_note, dodour, dodour_note, dcurling, dcurling_note, tgl_buat, tgl_update
			)
			VALUES
			(
				'$rcek[id]', '$_POST[dflamability]', '$_POST[dfla_note]', '$_POST[dfc_cott]', '$_POST[dfc_poly]', 
				'$_POST[dfc_ela]', '$_POST[dfc_cott1]', '$_POST[dfc_poly1]', '$_POST[dfc_ela1]', '$_POST[std_dfc_cott1]', 
				'$_POST[std_dfc_poly1]', '$_POST[std_dfc_elastane1]', '$_POST[dfibercontent]', '$_POST[dfiber_note]', 
				'$_POST[dwpi]', '$_POST[dcpi]', '$_POST[dfc_note]', '$_POST[dfabric_weight]', '$_POST[dfwe_note]', 
				'$_POST[dfabric_width]', '$_POST[dfwi_note]', '$_POST[dbow]', '$_POST[dskew]', '$_POST[dbas_note]', 
				'$_POST[dh_shrinkage_temp]', '$_POST[dh_shrinkage_l1]', '$_POST[dh_shrinkage_w1]', '$_POST[dh_shrinkage_grd]', 
				'$_POST[dh_shrinkage_app]', '$_POST[dh_shrinkage_note]', '$_POST[dss_temp]', '$_POST[dss_washes3]', 
				'$_POST[dss_washes10]', '$_POST[dss_washes15]', '$_POST[dss_cmt]', '$_POST[dshrinkage_len1]', 
				'$_POST[dshrinkage_len2]', '$_POST[dshrinkage_len3]', '$_POST[dshrinkage_len4]', '$_POST[dshrinkage_len5]', 
				'$_POST[dshrinkage_len6]', '$_POST[dshrinkage_wid1]', '$_POST[dshrinkage_wid2]', '$_POST[dshrinkage_wid3]', 
				'$_POST[dshrinkage_wid4]', '$_POST[dshrinkage_wid5]', '$_POST[dshrinkage_wid6]', '$_POST[dspirality1]', 
				'$_POST[dspirality2]', '$_POST[dspirality3]', '$_POST[dspirality4]', '$_POST[dspirality5]', '$_POST[dspirality6]', 
				'$_POST[dss_linedry]', '$_POST[dss_tumbledry]', '$_POST[dsns_note]', '$_POST[dapperss_ch1]', 
				'$_POST[dapperss_ch2]', '$_POST[dapperss_ch3]', '$_POST[dapperss_ch4]', '$_POST[dapperss_cc1]', 
				'$_POST[dapperss_cc2]', '$_POST[dapperss_cc3]', '$_POST[dapperss_cc4]', '$_POST[dapperss_st]', 
				...
				'$_POST[dapperss_note]', '$_POST[dpillingm_f1]', '$_POST[dpillingm_b1]', '$_POST[dpillingm_f2]', 
				'$_POST[dpillingm_b2]', '$_POST[dpillingm_f3]', '$_POST[dpillingm_b3]', '$_POST[dpillingm_f4]', 
				'$_POST[dpillingm_b4]', '$_POST[dpillingm_f5]', '$_POST[dpillingm_b5]', '$_POST[dpillm_note]', 
				'$_POST[dpillingl_f1]', '$_POST[dpillingl_b1]', '$_POST[dpillingl_f2]', '$_POST[dpillingl_b2]', 
				'$_POST[dpillingl_f3]', '$_POST[dpillingl_b3]', '$_POST[dpillingl_f4]', '$_POST[dpillingl_b4]', 
				'$_POST[dpillingl_f5]', '$_POST[dpillingl_b5]', '$_POST[dpilll_note]', '$_POST[dpillingb_f1]', 
				'$_POST[dpillingb_b1]', '$_POST[dpillingb_f2]', '$_POST[dpillingb_b2]', '$_POST[dpillingb_f3]', 
				'$_POST[dpillingb_b3]', '$_POST[dpillingb_f4]', '$_POST[dpillingb_b4]', '$_POST[dpillingb_f5]', 
				'$_POST[dpillingb_b5]', '$_POST[dpillb_note]', '$_POST[dpillingrt_f1]', '$_POST[dpillingrt_b1]', 
				'$_POST[dpillingrt_f2]', '$_POST[dpillingrt_b2]', '$_POST[dpillingrt_f3]', '$_POST[dpillingrt_b3]', 
				'$_POST[dpillingrt_f4]', '$_POST[dpillingrt_b4]', '$_POST[dpillingrt_f5]', '$_POST[dpillingrt_b5]', 
				'$_POST[dpillr_note]', '$_POST[dabr]', '$_POST[dabr_note]', '$_POST[dsnaggingm_l1]', 
				'$_POST[dsnaggingm_w1]', '$_POST[dsnaggingm_l2]', '$_POST[dsnaggingm_w2]', '$_POST[dsnaggingm_l3]', 
				'$_POST[dsnaggingm_w3]', '$_POST[dsnaggingm_l4]', '$_POST[dsnaggingm_w4]', '$_POST[dsnam_note]', 
				...
				'$_POST[dsnap_note]', '$_POST[dsnaggingb_l1]', '$_POST[dsnaggingb_w1]', '$_POST[dsnaggingb_l2]', 
				'$_POST[dsnaggingb_w2]', '$_POST[dsnaggingb_l3]', '$_POST[dsnaggingb_w3]', '$_POST[dsnaggingb_l4]', 
				'$_POST[dsnaggingb_w4]', '$_POST[dsnab_note]', '$_POST[dinstron]', '$_POST[dmullen]', 
				'$_POST[dtru_burst]', '$_POST[dburs_note]', '$_POST[dthick1]', '$_POST[dthick2]', '$_POST[dthick3]', 
				'$_POST[dthickav]', '$_POST[dthick_note]', '$_POST[dstretch_l1]', '$_POST[dstretch_w1]', 
				'$_POST[dstretch_l2]', '$_POST[dstretch_w2]', '$_POST[dstretch_l3]', '$_POST[dstretch_w3]', 
				'$_POST[dstretch_l4]', '$_POST[dstretch_w4]', '$_POST[dstretch_l5]', '$_POST[dstretch_w5]', 
				'$_POST[dload_stretch]', '$_POST[dstretch_note]', '$_POST[drecover_l1]', '$_POST[drecover_w1]', 
				'$_POST[drecover_l2]', '$_POST[drecover_w2]', '$_POST[drecover_l3]', '$_POST[drecover_w3]', 
				'$_POST[drecover_l4]', '$_POST[drecover_w4]', '$_POST[drecover_l5]', '$_POST[drecover_w5]', 
				'$_POST[drecover_note]', '$_POST[dgrowth_l1]', '$_POST[dgrowth_w1]', '$_POST[dgrowth_l2]', 
				'$_POST[dgrowth_w2]', '$_POST[drec_growth_l1]', '$_POST[drec_growth_w1]', '$_POST[drec_growth_l2]', 
				'$_POST[drec_growth_w2]', '$_POST[dgrowth_note]', '$_POST[dapper_ch1]', '$_POST[dapper_ch2]', 
				'$_POST[dapper_ch3]', '$_POST[dapper_cc1]', '$_POST[dapper_cc2]', '$_POST[dapper_cc3]', '$_POST[dapper_st]', 
				'$_POST[dapper_st2]', '$_POST[dapper_st3]', '$_POST[dapper_pf1]', '$_POST[dapper_pf2]', '$_POST[dapper_pf3]', 
				'$_POST[dapper_pb1]', '$_POST[dapper_pb2]', '$_POST[dapper_pb3]', '$_POST[dapper_acetate]', 
				'$_POST[dapper_cotton]', '$_POST[dapper_nylon]', '$_POST[dapper_poly]', '$_POST[dapper_acrylic]', 
				'$_POST[dapper_wool]', '$_POST[dapper_note]', '$_POST[dfibre_transfer]', '$_POST[dfibre_grade]', 
				'$_POST[dfibre_note]', '$_POST[dodour]', '$_POST[dodour_note]', '$_POST[dcurling]', '$_POST[dcurling_note]', 
				GETDATE(), GETDATE()
			)
		");
				echo "<script>swal({
...
if($_POST['colorfastness_save']=="save" and $cekNew>0){
	...
	if($sqlCLR){
		...
		$sqlCLRDI=sqlsrv_query($con_db_qc_sqlsrv,"INSERT INTO db_qc.tbl_tq_disptest 
			(
				id_nokk, dwash_temp, dwash_colorchange, dwash_acetate, dwash_cotton, dwash_nylon, dwash_poly, dwash_acrylic, 
				dwash_wool, dwash_staining, dwash_note, dwater_colorchange, dwater_acetate, dwater_cotton, dwater_nylon, 
				dwater_poly, dwater_acrylic, dwater_wool, dwater_staining, dwater_note, dacid_colorchange, dacid_acetate, 
				dacid_cotton, dacid_nylon, dacid_poly, dacid_acrylic, dacid_wool, dacid_staining, dacid_note, 
				dalkaline_colorchange, dalkaline_acetate, dalkaline_cotton, dalkaline_nylon, dalkaline_poly, dalkaline_acrylic, 
				dalkaline_wool, dalkaline_staining, dalkaline_note, dcrock_len1, dcrock_wid1, dcrock_len2, dcrock_wid2, 
				dcrock_note, dphenolic_colorchange, dphenolic_note, dcm_printing_colorchange, dcm_printing_staining, 
				dcm_printing_note, dcm_dye_temp, dcm_dye_colorchange, dcm_dye_stainingface, dcm_dye_stainingback, 
				dcm_dye_note, dlight_rating1, dlight_rating2, dlight_note, dlight_pers_colorchange, dlight_pers_note, 
				dsaliva_staining, dsaliva_note, dbleeding, dbleeding_note, dchlorin, dnchlorin1, dnchlorin2, dchlorin_note, 
				ddye_tf_sstaining, ddye_tf_cstaining, ddye_tf_acetate, ddye_tf_cotton, ddye_tf_nylon, ddye_tf_poly, 
				ddye_tf_acrylic, ddye_tf_wool, ddye_tf_note, tgl_buat, tgl_update
			)
			VALUES
			(
				'$rcek[id]', '$_POST[dwash_temp]', '$_POST[dwash_colorchange]', '$_POST[dwash_acetate]', 
				'$_POST[dwash_cotton]', '$_POST[dwash_nylon]', '$_POST[dwash_poly]', '$_POST[dwash_acrylic]', 
				'$_POST[dwash_wool]', '$_POST[dwash_staining]', '$_POST[dwash_note]', '$_POST[dwater_colorchange]', 
				'$_POST[dwater_acetate]', '$_POST[dwater_cotton]', '$_POST[dwater_nylon]', '$_POST[dwater_poly]', 
				'$_POST[dwater_acrylic]', '$_POST[dwater_wool]', '$_POST[dwater_staining]', '$_POST[dwater_note]', 
				'$_POST[dacid_colorchange]', '$_POST[dacid_acetate]', '$_POST[dacid_cotton]', '$_POST[dacid_nylon]', 
				'$_POST[dacid_poly]', '$_POST[dacid_acrylic]', '$_POST[dacid_wool]', '$_POST[dacid_staining]', 
				'$_POST[dacid_note]', '$_POST[dalkaline_colorchange]', '$_POST[dalkaline_acetate]', 
				'$_POST[dalkaline_cotton]', '$_POST[dalkaline_nylon]', '$_POST[dalkaline_poly]', '$_POST[dalkaline_acrylic]', 
				'$_POST[dalkaline_wool]', '$_POST[dalkaline_staining]', '$_POST[dalkaline_note]', '$_POST[dcrock_len1]', 
				'$_POST[dcrock_wid1]', '$_POST[dcrock_len2]', '$_POST[dcrock_wid2]', '$_POST[dcrock_note]', 
				'$_POST[dphenolic_colorchange]', '$_POST[dphenolic_note]', '$_POST[dcm_printing_colorchange]', 
				'$_POST[dcm_printing_staining]', '$_POST[dcm_printing_note]', '$_POST[dcm_dye_temp]', 
				'$_POST[dcm_dye_colorchange]', '$_POST[dcm_dye_stainingface]', '$_POST[dcm_dye_stainingback]', 
				'$_POST[dcm_dye_note]', '$_POST[dlight_rating1]', '$_POST[dlight_rating2]', '$_POST[dlight_note]', 
				'$_POST[dlight_pers_colorchange]', '$_POST[dlight_pers_note]', '$_POST[dsaliva_staining]', 
				'$_POST[dsaliva_note]', '$_POST[dbleeding]', '$_POST[dbleeding_note]', '$_POST[dchlorin]', 
				'$_POST[dnchlorin1]', '$_POST[dnchlorin2]', '$_POST[dchlorin_note]', '$_POST[ddye_tf_sstaining]', 
				'$_POST[ddye_tf_cstaining]', '$_POST[ddye_tf_acetate]', '$_POST[ddye_tf_cotton]', '$_POST[ddye_tf_nylon]', 
				'$_POST[ddye_tf_poly]', '$_POST[ddye_tf_acrylic]', '$_POST[ddye_tf_wool]', '$_POST[ddye_tf_note]', 
				GETDATE(), GETDATE()
			)
		");
	...
	$sqlCLR=sqlsrv_query($con_db_qc_sqlsrv,"INSERT INTO db_qc.tbl_tq_test 
		(
			id_nokk, wash_temp, wash_colorchange, wash_acetate, wash_cotton, wash_nylon, wash_poly, wash_acrylic, 
			wash_wool, wash_staining, wash_note, water_colorchange, water_acetate, water_cotton, water_nylon, water_poly, 
			water_acrylic, water_wool, water_staining, water_note, acid_colorchange, acid_acetate, acid_cotton, acid_nylon, 
			acid_poly, acid_acrylic, acid_wool, acid_staining, acid_note, alkaline_colorchange, alkaline_acetate, 
			alkaline_cotton, alkaline_nylon, alkaline_poly, alkaline_acrylic, alkaline_wool, alkaline_staining, alkaline_note, 
			crock_len1, crock_wid1, crock_len2, crock_wid2, crock_note, phenolic_colorchange, phenolic_note, 
			cm_printing_colorchange, cm_printing_staining, cm_printing_note, cm_dye_temp, cm_dye_colorchange, 
			cm_dye_stainingface, cm_dye_stainingback, cm_dye_note, light_rating1, light_rating2, light_note, 
			light_pers_colorchange, light_pers_note, saliva_staining, saliva_note, bleeding, bleeding_note, chlorin, 
			nchlorin1, nchlorin2, chlorin_note, dye_tf_sstaining, dye_tf_cstaining, dye_tf_acetate, dye_tf_cotton, 
			dye_tf_nylon, dye_tf_poly, dye_tf_acrylic, dye_tf_wool, dye_tf_note, stat_wf, stat_wtr, stat_pac, stat_pal, 
			stat_cr, stat_py, stat_cmo, stat_cm, stat_lg, stat_lp, stat_slv, stat_bld, stat_chl, stat_nchl, stat_dye, 
			tgl_buat, tgl_update
		)
		VALUES
		(
			'$rcek[id]', '$_POST[wash_temp]', '$_POST[wash_colorchange]', '$_POST[wash_acetate]', '$_POST[wash_cotton]', 
			'$_POST[wash_nylon]', '$_POST[wash_poly]', '$_POST[wash_acrylic]', '$_POST[wash_wool]', '$_POST[wash_staining]', 
			'$_POST[wash_note]', '$_POST[water_colorchange]', '$_POST[water_acetate]', '$_POST[water_cotton]', 
			'$_POST[water_nylon]', '$_POST[water_poly]', '$_POST[water_acrylic]', '$_POST[water_wool]', 
			'$_POST[water_staining]', '$_POST[water_note]', '$_POST[acid_colorchange]', '$_POST[acid_acetate]', 
			'$_POST[acid_cotton]', '$_POST[acid_nylon]', '$_POST[acid_poly]', '$_POST[acid_acrylic]', '$_POST[acid_wool]', 
			'$_POST[acid_staining]', '$_POST[acid_note]', '$_POST[alkaline_colorchange]', '$_POST[alkaline_acetate]', 
			'$_POST[alkaline_cotton]', '$_POST[alkaline_nylon]', '$_POST[alkaline_poly]', '$_POST[alkaline_acrylic]', 
			'$_POST[alkaline_wool]', '$_POST[alkaline_staining]', '$_POST[alkaline_note]', '$_POST[crock_len1]', 
			'$_POST[crock_wid1]', '$_POST[crock_len2]', '$_POST[crock_wid2]', '$_POST[crock_note]', 
			'$_POST[phenolic_colorchange]', '$_POST[phenolic_note]', '$_POST[cm_printing_colorchange]', 
			'$_POST[cm_printing_staining]', '$_POST[cm_printing_note]', '$_POST[cm_dye_temp]', '$_POST[cm_dye_colorchange]', 
			'$_POST[cm_dye_stainingface]', '$_POST[cm_dye_stainingback]', '$_POST[cm_dye_note]', '$_POST[light_rating1]', 
			'$_POST[light_rating2]', '$_POST[light_note]', '$_POST[light_pers_colorchange]', '$_POST[light_pers_note]', 
			'$_POST[saliva_staining]', '$_POST[saliva_note]', '$_POST[bleeding]', '$_POST[bleeding_note]', '$_POST[chlorin]', 
			'$_POST[nchlorin1]', '$_POST[nchlorin2]', '$_POST[chlorin_note]', '$_POST[dye_tf_sstaining]', 
			'$_POST[dye_tf_cstaining]', '$_POST[dye_tf_acetate]', '$_POST[dye_tf_cotton]', '$_POST[dye_tf_nylon]', 
			...
			'$_POST[stat_bld]', '$_POST[stat_chl]', '$_POST[stat_nchl]', '$_POST[stat_dye]', GETDATE(), GETDATE()
		)
	");
...
if($_POST['functional_save']=="save" and $cekNew>0){
	...
	$sqlFPHDI=sqlsrv_query($con_db_qc_sqlsrv,"INSERT INTO db_qc.tbl_tq_disptest 
		(
			id_nokk, dwick_l1, dwick_w1, dwick_l2, dwick_w2, dwick_l3, dwick_w3, dwick_l4, dwick_w4, dwick_note, 
			dabsor_f1, dabsor_f2, dabsor_f3, dabsor_b1, dabsor_b2, dabsor_b3, dabsor_note, ddry1, ddry2, ddry3, 
			ddryaf1, ddryaf2, ddryaf3, ddry_note, drepp1, drepp2, drepp3, drepp4, drepp_note, dph, dph_note, 
			dsoil, dsoil_note, dhumidity, dhumidity_note, tgl_buat, tgl_update
		)
		VALUES
		(
			'$rcek[id]', '$_POST[dwick_l1]', '$_POST[dwick_w1]', '$_POST[dwick_l2]', '$_POST[dwick_w2]', 
			'$_POST[dwick_l3]', '$_POST[dwick_w3]', '$_POST[dwick_l4]', '$_POST[dwick_w4]', '$_POST[dwick_note]', 
			'$_POST[dabsor_f1]', '$_POST[dabsor_f2]', '$_POST[dabsor_f3]', '$_POST[dabsor_b1]', '$_POST[dabsor_b2]', 
			'$_POST[dabsor_b3]', '$_POST[dabsor_note]', '$_POST[ddry1]', '$_POST[ddry2]', '$_POST[ddry3]', 
			'$_POST[ddryaf1]', '$_POST[ddryaf2]', '$_POST[ddryaf3]', '$_POST[ddry_note]', '$_POST[drepp1]', 
			'$_POST[drepp2]', '$_POST[drepp3]', '$_POST[drepp4]', '$_POST[drepp_note]', '$_POST[dph]', 
			'$_POST[dph_note]', '$_POST[dsoil]', '$_POST[dsoil_note]', '$_POST[dhumidity]', '$_POST[dhumidity_note]', 
			GETDATE(), GETDATE()
		)
	");
	echo "<script>swal({
	...
	$sqlFPH=sqlsrv_query($con_db_qc_sqlsrv,"INSERT INTO db_qc.tbl_tq_test 
		(
			id_nokk, wick_l1, wick_w1, wick_l2, wick_w2, wick_l3, wick_w3, wick_l4, wick_w4, wick_note, 
			absor_f1, absor_f2, absor_f3, absor_b1, absor_b2, absor_b3, absor_note, dry1, dry2, dry3, 
			dryaf1, dryaf2, dryaf3, dry_note, repp1, repp2, repp3, repp4, repp_note, ph, ph_note, 
			soil, soil_note, humidity, humidity_note, stat_wic, stat_wic1, stat_wic2, stat_wic3, stat_abs, 
			stat_abs1, stat_dry, stat_dry1, stat_wp, stat_wp1, stat_ph, stat_sor, stat_hum, tgl_buat, tgl_update
		)
		VALUES
		(
			'$rcek[id]', '$_POST[wick_l1]', '$_POST[wick_w1]', '$_POST[wick_l2]', '$_POST[wick_w2]', 
			'$_POST[wick_l3]', '$_POST[wick_w3]', '$_POST[wick_l4]', '$_POST[wick_w4]', '$_POST[wick_note]', 
			'$_POST[absor_f1]', '$_POST[absor_f2]', '$_POST[absor_f3]', '$_POST[absor_b1]', '$_POST[absor_b2]', 
			'$_POST[absor_b3]', '$_POST[absor_note]', '$_POST[dry1]', '$_POST[dry2]', '$_POST[dry3]', 
			'$_POST[dryaf1]', '$_POST[dryaf2]', '$_POST[dryaf3]', '$_POST[dry_note]', '$_POST[repp1]', 
			'$_POST[repp2]', '$_POST[repp3]', '$_POST[repp4]', '$_POST[repp_note]', '$_POST[ph]', 
			'$_POST[ph_note]', '$_POST[soil]', '$_POST[soil_note]', '$_POST[humidity]', '$_POST[humidity_note]', 
			'$_POST[stat_wic]', '$_POST[stat_wic1]', '$_POST[stat_wic2]', '$_POST[stat_wic3]', '$_POST[stat_abs]', 
			'$_POST[stat_abs1]', '$_POST[stat_dry]', '$_POST[stat_dry1]', '$_POST[stat_wp]', '$_POST[stat_wp1]', 
			'$_POST[stat_ph]', '$_POST[stat_sor]', '$_POST[stat_hum]', GETDATE(), GETDATE()
		)
	");

	if($sqlFPH){
		$sqlFPHD=sqlsrv_query($con_db_qc_sqlsrv,"UPDATE tbl_tq_disptest SET
			dwick_l1 = '$_POST[dwick_l1]',
			dwick_w1 = '$_POST[dwick_w1]',
			dwick_l2 = '$_POST[dwick_l2]',
			dwick_w2 = '$_POST[dwick_w2]',
			...
			WHERE id_nokk = '$rcek[id]'
		");
		$sqlFPHDI=sqlsrv_query($con_db_qc_sqlsrv,"INSERT INTO tbl_tq_disptest 
			(
				id_nokk, dwick_l1, dwick_w1, dwick_l2, dwick_w2, dwick_l3, dwick_w3, dwick_l4, dwick_w4, dwick_note, 
				dabsor_f1, dabsor_f2, dabsor_f3, dabsor_b1, dabsor_b2, dabsor_b3, dabsor_note, ddry1, ddry2, ddry3, 
				ddryaf1, ddryaf2, ddryaf3, ddry_note, drepp1, drepp2, drepp3, drepp4, drepp_note, dph, dph_note, 
		...
		echo "<script>swal({
			...
			echo "<script>swal({
			title: 'Functional Save Gagal Tersimpan',   
			text: 'Klik Ok untuk input data kembali',
			type: 'error',
			}).then((result) => {
			if (result.value) {
...
<?php if (isset($_POST['update_first_lot'])) { ?>
	...
	$sqlCekNew = sqlsrv_query($con_db_qc_sqlsrv, "SELECT TOP 1 a.* 
			FROM $tabel_sumber a
			JOIN db_qc.tbl_tq_nokk b on (a.id_nokk = b.id)
			WHERE b.nodemand = '$nodemand' order by b.id desc");
	$rcekNew = sqlsrv_fetch_array($sqlCekNew);

	if ($rcekNew) {
	
		$sqlCekFl = sqlsrv_query($con_db_qc_sqlsrv, "SELECT TOP 1 a.id as pk, b.id as pk_test , b.id_nokk
				FROM db_qc.tbl_tq_first_lot a
				LEFT JOIN $tabel_tujuan  b on (a.id = b.id_nokk)
				WHERE a.nodemand = '$nodemand' order by a.id desc");
    $rcekFl = sqlsrv_fetch_array($sqlCekFl);
    ...
    if ($rcekFl['id_nokk'] != null) {
        ...
        foreach ($rcekNew as $key => $value) {
            if (is_string($key) && $key != 'id' && $key != 'id_nokk') { // Skip 'id' column
                $escapedValue = $value; // Escape the value
                $updateSql .= "$key = '$escapedValue', ";
            }
        }
        $updateSql = rtrim($updateSql, ', ') . " WHERE id = '$id'";
		   
        $return 	 =  sqlsrv_query($con_db_qc_sqlsrv, $updateSql);
		...
		return true;
			...
			$return 	 = sqlsrv_query($con_db_qc_sqlsrv, $insertSql);
	...
	if ($tq_test or $tq_marginal or $disptest) {
		echo "<script>
					alert('Data Berhasil Diupdate FirstLost $nodemand'); 
			 </script>";
	} else {
		echo "<script>
