...
$explode = explode("00000",$_GET['no_item']);
$noitem = $explode[0];
$pelanggan = $explode[1] ; 
...
if($_POST['save']=="save"){
	...
	if($sqlKK){
		...
		if (result.value) {
			...
			window.location.href='StdTQFL-$r[no_item]'; 
		}
		});</script>";
	}
}
...
<form class="form-horizontal" action="" method="post" enctype="multipart/form-data" name="form0" id="form0">
    <div class="box box-success" style="width: 98%;">
        ...
        <script>
				function handleChange() {
					var input = document.getElementById("no_item");
					var modifiedValue = input.value.replaceAll("/", "00000").replaceAll(" ", "77777");
                    var pelanggan = "<?php echo $pelanggan; ?>"
					window.location = "StdTQFL-" + modifiedValue + "00000" + pelanggan ;
				}
		</script>
                ...
                <div class="form-group">
                    <label for="no_item" class="col-sm-3 control-label">No Item</label>
                        <div class="col-sm-5">
                            ...
                            onchange="handleChange()" value="<?php if($_GET['no_item']!=""){echo $noitem; }?>" placeholder="No Item" required >
                        </div>
                </div>
            </div> 
        </div>
    ...
    <?php if($noitem!="" and $noitem !='false'){ ?> 
        <div class="box box-success">
            <div class="box-header with-border">
                ...
                <h3 class="box-title" style="color:<?=$color?>">Detail Standard Under Armour
