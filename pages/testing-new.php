...
<script>
	...
	function tampil1() {
		...
		if (document.forms['form3']['jns_test1'].value == "WATER REPPELENT") {
			...
			if (document.forms['form3']['stat_wp'].value == "MARGINAL PASS") {//kesana
			...
			<div class="box-body">
				<div class="col-md-6">
					<div class="form-group">
						...
						<!-- <?php if ($_GET['nodemand'] != "") { ?><a href="#" class="btn btn-info btn-xs posisi_kktq" id="<?php echo $_GET['nokk']; ?>">Posisi Terakhir</a><?php } ?> -->
					</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-5">
								<input name="nodemand_old" type="text" class="form-control" id="nodemand_old"
									placeholder="No Demand Old" readonly="readonly">
							</div>
						</div>
					<?php } ?>
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input name="kk_legacy" type="text" class="form-control" id="kk_legacy"
								placeholder="No KK Legacy" readonly="readonly">
						</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<textarea name="jns_kain" readonly="readonly" class="form-control" id="jns_kain"
								placeholder="Jenis Kain"></textarea>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-5">
							<input name="lot_legacy" type="text" class="form-control" id="lot_legacy"
								placeholder="No Lot Legacy" readonly="readonly">
						</div>
					</div>
					<div class="form-group">
...
$sqlCek = sqlsrv_query(
    ...
    "SELECT TOP 1 a.*
     ...
     WHERE COALESCE(NULLIF(a.nodemand_new, ''), a.nodemand) = ?
     ORDER BY a.id DESC",
    [$nodemand]
);
if ($sqlCek === false) {
    echo "<pre>SQLSRV ERROR (sqlCek 1):\n";
...
// $meta = sqlsrv_field_metadata($sqlCek);
// print_r($meta);
...
if ($cek == 0) {
    $sqlCek = sqlsrv_query(
        $con_db_qc_sqlsrv,
        "SELECT TOP 1 b.*
         FROM db_qc.tbl_tq_nokk_demand a
    ...
    if ($sqlCek === false) {
        echo "<pre>SQLSRV ERROR (sqlCek 2):\n";
        print_r(sqlsrv_errors());
        echo "</pre>";
        die;
    }
...
$nokk_demand_sql = sqlsrv_query(
    $con_db_qc_sqlsrv,
    "SELECT TOP 1 *
     FROM db_qc.tbl_tq_nokk_demand
     WHERE id_nokk = ? AND id_nokk > 0
     ORDER BY id DESC",
    [$id_nokk]
);

if ($nokk_demand_sql === false) {
    echo "<pre>SQLSRV ERROR (nokk_demand_sql):\n";
    print_r(sqlsrv_errors());
    echo "</pre>";
    die;
}

$rows_nokk_demand = sqlsrv_fetch_array($nokk_demand_sql, SQLSRV_FETCH_ASSOC);
$nokk_demand_data = ($rows_nokk_demand) ? 1 : 0;
...
if ($nokk_demand_data > 0) {
    $id_nokk = $rows_nokk_demand['id_nokk'];

    $demand_other = sqlsrv_query(
        $con_db_qc_sqlsrv,
        "SELECT *
         FROM db_qc.tbl_tq_nokk_demand
         WHERE id_nokk = ?
    ...
    if ($demand_other === false) {
        echo "<pre>SQLSRV ERROR (demand_other):\n";
        print_r(sqlsrv_errors());
        echo "</pre>";
        die;
    }
		...
		<div class="box-body">
			<div class="col-md-6">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input name="no_test" type="text" class="form-control" id="no_test" placeholder="No Test"
							autocomplete="off" onchange="window.location='TestingNewNoTes-'+this.value" value="<?php if ($cek > 0) {
								echo $rcek['no_test'];
							} else {
								echo $_GET['no_test'];
							} ?>">
...
$sqlCek1 = sqlsrv_query($con_db_qc_sqlsrv, "SELECT TOP 1 a.*, b.*,
    ...
    ON TRY_CONVERT(BIGINT, a.id_nokk) = TRY_CONVERT(BIGINT, b.id_nokk)
WHERE TRY_CONVERT(BIGINT, a.id_nokk) ='$rcek[id]' 
	ORDER BY a.id DESC");

if ($sqlCek1 === false) {
    echo "<pre>SQL ERROR sqlCek1:\n";
    print_r(sqlsrv_errors());
    echo "</pre>";
    die;
}
...
//Spirality Status 
/*
$sqlCmt=sqlsrv_query($con,"SELECT *,
...
$tq_test_2_sql = sqlsrv_query($con_db_qc_sqlsrv, " SELECT id_nokk, spirality_status, bleeding_root, wrinkle, wrinkle1, wrinkle2, stat_wrinkle, stat_wrinkle1, wrinkle_note from db_qc.tbl_tq_test_2 where id_nokk = '$id_tq_test_2'");
$tq_test_2_array = sqlsrv_fetch_array($tq_test_2_sql);
?>
						...
						<div class="tab-pane active" id="tab_1">
								...
								<div class="form-group">
									...
									<div class="col-sm-4">
										<select name="jns_test" class="form-control select2" id="jns_test"
											...
											<?php
											$sql = "SELECT a.*, b.* From db_qc.tbl_tq_nokk a INNER JOIN db_qc.tbl_master_test b ON a.no_test=b.no_testmaster WHERE no_test='$rcek[no_test]'";
							...
							<div class="col-sm-2">
								...
								<div class="form-group" id="fc7" style="display:none;">
									...
									<div class="col-sm-1">
										...
										// $tq_test_2_sql	 = sqlsrv_query($con,"select id_nokk, spirality_status, bleeding_root from tbl_tq_test_2 where id_nokk = '$id_tq_test_2'");
										// $tq_test_2_array = sqlsrv_fetch_array($tq_test_2_sql);
							...
							<div class="col-sm-1">
								...
								<div class="form-group" id="ranabr" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="rabr_note" maxlength="50" rows="1"
											readonly><?php echo $rcekR['rabr_note']; ?></textarea>
									</div>
								</div>
								<!-- ABRATION END-->
								<!-- SNAGGING MACE BEGIN-->
								<div class="form-group" id="fc12" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="snam_note" maxlength="50"><?php echo $rcek1['snam_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="stat_sm" style="display:none;">
									<label for="stat_sm" class="col-sm-2 control-label">STATUS</label>
									<div class="col-sm-2">
										<select name="stat_sm" class="form-control select2" id="stat_sm"
											onChange="tampil();" style="width: 100%;">
								...
								<div class="form-group" id="ransm" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="rsnam_note" maxlength="50"
											readonly><?php echo $rcekR['rsnam_note']; ?></textarea>
									</div>
								</div>
								<!-- SNAGGING MACE END-->
								<!-- SNAGGING POD BEGIN-->
								<div class="form-group" id="fc13" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="snap_note" maxlength="50"><?php echo $rcek1['snap_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="stat_sp" style="display:none;">
									<label for="stat_sp" class="col-sm-2 control-label">STATUS</label>
									<div class="col-sm-2">
										<select name="stat_sp" class="form-control select2" id="stat_sp"
											onChange="tampil();" style="width: 100%;">
								...
								<div class="form-group" id="ransp" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="rsnap_note" maxlength="50"
											readonly><?php echo $rcekR['rsnap_note']; ?></textarea>
									</div>
								</div>
								<!-- SNAGGING POD END-->
								<!-- BEAN BAG BEGIN-->
							...
							-->
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="snab_note" maxlength="50"><?php echo $rcek1['snab_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="stat_sb" style="display:none;">
									<label for="stat_sb" class="col-sm-2 control-label">STATUS</label>
									<div class="col-sm-2">
										<select name="stat_sb" class="form-control select2" id="stat_sb"
											onChange="tampil();" style="width: 100%;">
							...
							-->
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="rsnab_note" maxlength="50"
											readonly><?php echo $rcekR['rsnab_note']; ?></textarea>
									</div>
								</div>
								<!-- BEAN BAG END-->
								...
								<div class="form-group" id="fc15" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_bs2" class="form-control select2" id="stat_bs2"
											onChange="tampil();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_bs2'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="burs_note" maxlength="50"
											rows="1"><?php echo $rcek1['burs_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="disbs2" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dburs_note" maxlength="50"
											rows="1"><?php echo $rcekD['dburs_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="marbs2" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="mburs_note" maxlength="50"
											rows="1"><?php echo $rcekM['mburs_note']; ?></textarea>
									</div>
								</div>
								...
								<div class="form-group" id="fc26" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_bs3" class="form-control select2" id="stat_bs3"
											onChange="tampil();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_bs3'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="fc25" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_bs" class="form-control select2" id="stat_bs"
											onChange="tampil();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_bs'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="fc16" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="thick_note" maxlength="50"
											rows="1"><?php echo $rcek1['thick_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="stat_th" style="display:none;">
									<label for="stat_th" class="col-sm-2 control-label">STATUS</label>
									<div class="col-sm-2">
										<select name="stat_th" class="form-control select2" id="stat_th"
											onChange="tampil();" style="width: 100%;">
								...
								<div class="form-group" id="fc17" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="stretch_note"
											maxlength="50"><?php echo $rcek1['stretch_note']; ?></textarea>
									</div>
								</div>
								...
								<div class="form-group" id="stat_sr" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_sr" class="form-control select2" id="stat_sr"
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_sr'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="disst" style="display:none;">
									...
									<div class="col-sm-1">
										<label><input type="checkbox" name="dload_stretch" id="dload_stretch"
												class="minimal" value="60" <?php if ($rcekD['dload_stretch'] == '60') {
													echo "checked";
												} ?>> 60N &nbsp; &nbsp; &nbsp; &nbsp;
										</label>
										<label><input type="checkbox" name="dload_stretch" id="dload_stretch"
												class="minimal" value="22" <?php if ($rcekD['dload_stretch'] == '22') {
													echo "checked";
												} ?>> 22N
										</label>
									</div>
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dstretch_note"
											maxlength="50"><?php echo $rcekD['dstretch_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="marst" style="display:none;">
									<label for="marst" class="col-sm-2 control-label">STRETCH (MARGINAL)</label>
									<div class="col-sm-1">
										<label><input type="checkbox" name="mload_stretch" id="mload_stretch"
												class="minimal" value="60" <?php if ($rcekM['mload_stretch'] == '60') {
													echo "checked";
												} ?>> 60N &nbsp; &nbsp; &nbsp; &nbsp;
										</label>
										<label><input type="checkbox" name="mload_stretch" id="mload_stretch"
												class="minimal" value="22" <?php if ($rcekM['mload_stretch'] == '22') {
													echo "checked";
												} ?>> 22N
										</label>
									</div>
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="mstretch_note"
											maxlength="50"><?php echo $rcekM['mstretch_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="ranst" style="display:none;">
									<label for="ranst" class="col-sm-2 control-label">STRETCH (RAN)</label>
									<div class="col-sm-1">
										<label><input type="checkbox" name="rload_stretch" id="rload_stretch"
												class="minimal" value="60" <?php if ($rcekR['rload_stretch'] == '60') {
													echo "checked";
												} ?>> 60N &nbsp; &nbsp; &nbsp; &nbsp;
										</label>
										<label><input type="checkbox" name="rload_stretch" id="rload_stretch"
												class="minimal" value="22" <?php if ($rcekR['rload_stretch'] == '22') {
													echo "checked";
												} ?>> 22N
										</label>
									</div>
								...
								<div class="form-group" id="disrc" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="drecover_note"
											maxlength="50"><?php echo $rcekD['drecover_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="marrc" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="mrecover_note"
											maxlength="50"><?php echo $rcekM['mrecover_note']; ?></textarea>
									</div>
								</div>
								...
								<div class="form-group" id="fc19" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="growth_note"
											maxlength="50"><?php echo $rcek1['growth_note']; ?></textarea>
									</div>
								</div>
								...
								<div class="form-group" id="stat_gr" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_gr" class="form-control select2" id="stat_gr"
											onChange="tampil();" style="width: 100%;">
								...
								<div class="form-group" id="disgr" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dgrowth_note"
											maxlength="50"><?php echo $rcekD['dgrowth_note']; ?></textarea>
									</div>
								</div>
								...
								<div class="form-group" id="stat_ap" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_ap" class="form-control select2" id="stat_ap"
											onChange="tampil();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_ap'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="disap" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dapper_note"
											maxlength="50"><?php echo $rcekD['dapper_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="marap" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="mapper_note"
											maxlength="50"><?php echo $rcekM['mapper_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="ranap" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="rapper_pf1" type="text" class="form-control" id="rapper_pf1"
											value="<?php echo $rcekR['rapper_pf1']; ?>" placeholder="PILLING FACE 1"
											readonly>
										<input name="rapper_pb1" type="text" class="form-control" id="rapper_pb1"
											value="<?php echo $rcekR['rapper_pb1']; ?>" placeholder="PILLING BACK 1"
											readonly>
									...
									<div class="col-sm-2">
										<input name="rapper_pf2" type="text" class="form-control" id="rapper_pf2"
											value="<?php echo $rcekR['rapper_pf2']; ?>" placeholder="PILLING FACE 2"
											readonly>
										<input name="rapper_pb2" type="text" class="form-control" id="rapper_pb2"
											value="<?php echo $rcekR['rapper_pb2']; ?>" placeholder="PILLING BACK 2"
											readonly>
									...
									<div class="col-sm-2">
										<input name="rapper_pf3" type="text" class="form-control" id="rapper_pf3"
											value="<?php echo $rcekR['rapper_pf3']; ?>" placeholder="PILLING FACE 3"
											readonly>
										<input name="rapper_pb3" type="text" class="form-control" id="rapper_pb3"
											value="<?php echo $rcekR['rapper_pb3']; ?>" placeholder="PILLING BACK 3"
											readonly>
								...
								<div class="form-group" id="fc21" style="display:none;">
									...
									<div class="col-sm-1">
										<label><input type="checkbox" name="h_shrinkage_temp" id="h_shrinkage_temp"
												class="minimal" value="170" <?php if ($rcek1['h_shrinkage_temp'] == '170') {
													echo "checked";
												} ?>> 170&deg;C &nbsp; &nbsp; &nbsp; &nbsp;
										</label>
										<label><input type="checkbox" name="h_shrinkage_temp" id="h_shrinkage_temp"
												class="minimal" value="180" <?php if ($rcek1['h_shrinkage_temp'] == '180') {
													echo "checked";
												} ?>> 180&deg;C
										</label>
										<label><input type="checkbox" name="h_shrinkage_temp" id="h_shrinkage_temp"
												class="minimal" value="200" <?php if ($rcek1['h_shrinkage_temp'] == '200') {
													echo "checked";
												} ?>> 200&deg;C
										</label>
									</div>
								...
								<div class="form-group" id="stat_hs" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_hs" class="form-control select2" id="stat_hs"
											onChange="tampil();" style="width: 100%;">
								...
								<div class="form-group" id="dishs" style="display:none;">
									...
									<div class="col-sm-1">
										<input name="dh_shrinkage_grd" type="text" class="form-control"
											id="dh_shrinkage_grd" value="<?php echo $rcekD['dh_shrinkage_grd']; ?>"
											placeholder="GRADE">
									</div>
									<div class="col-sm-2">
										<input name="dh_shrinkage_app" type="text" class="form-control"
											id="dh_shrinkage_app" value="<?php echo $rcekD['dh_shrinkage_app']; ?>"
								...
								<div class="form-group" id="ranhs" style="display:none;">
									...
									<div class="col-sm-1">
										<input name="rh_shrinkage_grd" type="text" class="form-control"
											id="rh_shrinkage_grd" value="<?php echo $rcekR['rh_shrinkage_grd']; ?>"
											placeholder="GRADE" readonly>
									</div>
									<div class="col-sm-2">
										<input name="rh_shrinkage_app" type="text" class="form-control"
											id="rh_shrinkage_app" value="<?php echo $rcek1['rh_shrinkage_app']; ?>"
											placeholder="PASS/FAIL" readonly>
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="stat_ff" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_ff" class="form-control select2" id="stat_ff"
											onChange="tampil();" style="width: 100%;">
								...
								<div class="form-group" id="disff" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dfibre_note"
											maxlength="50"><?php echo $rcekD['dfibre_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="ranff" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="rfibre_transfer" type="text" class="form-control" id="rfibre_transfer"
											value="<?php echo $rcekR['rfibre_transfer']; ?>" placeholder="FIBRE TRANSFER"
											readonly>
									</div>
									<div class="col-sm-1">
								...
								<div class="form-group" id="stat_odour" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_odour" class="form-control select2" id="stat_odour"
											onChange="tampil();" style="width: 100%;">
											<option <?php if ($rcek1['stat_odour'] == "") { ?> selected=selected <?php }
											; ?>value="">Pilih</option>
											<option <?php if ($rcek1['stat_odour'] == "DISPOSISI") { ?> selected=selected
												<?php }
											; ?>value="DISPOSISI">DISPOSISI</option>
								...
								<div class="form-group" id="disod" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dodour_note"
											maxlength="50"><?php echo $rcekD['dodour_note']; ?></textarea>
									</div>
								</div>
								...
								<div class="form-group" id="fc29" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="curling_note"
											maxlength="50"><?php echo $rcek1['curling_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="stat_curling" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_curling" class="form-control select2" id="stat_curling"
											...
											; ?>value="">Pilih</option>
											<option <?php if ($rcek1['stat_curling'] == "DISPOSISI") { ?> selected=selected
												<?php }
											; ?>value="DISPOSISI">DISPOSISI</option>
								...
								<div class="form-group" id="discur" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dcurling_note"
											maxlength="50"><?php echo $rcekD['dcurling_note']; ?></textarea>
									</div>
								</div>
								...
								<div class="form-group" id="stat_nedle" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_nedle" class="form-control select2" id="stat_nedle"
											onChange="tampil();" style="width: 100%;">
											<option <?php if ($rcek1['stat_nedle'] == "") { ?> selected=selected <?php }
											; ?>value="">Pilih</option>
											<option <?php if ($rcek1['stat_nedle'] == "DISPOSISI") { ?> selected=selected
												<?php }
											; ?>value="DISPOSISI">DISPOSISI</option>
								...
								<div class="form-group" id="disned" style="display:none;">
									<label for="disned" class="col-sm-2 control-label">NEDLE HOLES &amp; CRACKING
										(DIS)</label>
									<div class="col-sm-2">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dnedle_note"
											maxlength="50"><?php echo $rcekD['dnedle_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="ranned" style="display:none;">
									<label for="ranned" class="col-sm-2 control-label">NEDLE HOLES &amp; CRACKING
										(RAN)</label>
									<div class="col-sm-2">
								...
								<div class="form-group" id="fc31" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_wrinkle" class="form-control select2" id="stat_wrinkle"
											onChange="tampil();" style="width: 100%;">
											<option <?php if ($tq_test_2_array['stat_wrinkle'] == "") { ?> selected=selected
												<?php }
											; ?>value="">Pilih</option>
											<!-- <option <?php //if($tq_test_2_array['stat_wrinkle']=="DISPOSISI"){      ?> selected=selected <?php //};      ?>value="DISPOSISI">DISPOSISI</option> -->
											<option <?php if ($tq_test_2_array['stat_wrinkle'] == "A") { ?> selected=selected
												<?php }
											; ?>value="A">A</option>
											<option <?php if ($tq_test_2_array['stat_wrinkle'] == "R") { ?> selected=selected
												<?php }
											; ?>value="R">R</option>
											<option <?php if ($tq_test_2_array['stat_wrinkle'] == "PASS") { ?>
													selected=selected <?php }
											; ?>value="PASS">PASS</option>
											<option <?php if ($tq_test_2_array['stat_wrinkle'] == "FAIL") { ?>
													selected=selected <?php }
											; ?>value="FAIL">FAIL</option>
								...
								<div class="form-group" id="fc32" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_wrinkle1" class="form-control select2" id="stat_wrinkle1"
											onChange="tampil();" style="width: 100%;">
											<option <?php if ($tq_test_2_array['stat_wrinkle1'] == "") { ?> selected=selected
												<?php }
											; ?>value="">Pilih</option>
											<!-- <option <?php //if($tq_test_2_array['stat_wrinkle1']=="DISPOSISI"){      ?> selected=selected <?php //};      ?>value="DISPOSISI">DISPOSISI</option> -->
											<option <?php if ($tq_test_2_array['stat_wrinkle1'] == "A") { ?> selected=selected
												<?php }
											; ?>value="A">A</option>
											<option <?php if ($tq_test_2_array['stat_wrinkle1'] == "R") { ?> selected=selected
												<?php }
											; ?>value="R">R</option>
											<option <?php if ($tq_test_2_array['stat_wrinkle1'] == "PASS") { ?>
													selected=selected <?php }
											; ?>value="PASS">PASS</option>
											<option <?php if ($tq_test_2_array['stat_wrinkle1'] == "FAIL") { ?>
													selected=selected <?php }
											; ?>value="FAIL">FAIL</option>
								...
								<div class="form-group">
									<?php if ($nodemand != "") { ?>
										<button type="submit" class="btn btn-primary pull-right" name="physical_save"
						...
						<div class="tab-pane" id="tab_2">
							<form class="form-horizontal" action="" method="post" enctype="multipart/form-data" name="form2"
								...
								<div class="form-group" id="c1" style="display:none;">
									...
									<div class="col-sm-1">
										<label><input type="checkbox" name="wash_temp" id="wash_temp" class="minimal"
												value="30" <?php if ($rcek1['wash_temp'] == '30') {
													echo "checked";
												} ?>>
											30&deg;C
											&nbsp; &nbsp; &nbsp; &nbsp;
										...
										<label><input type="checkbox" name="wash_temp" id="wash_temp" class="minimal"
												value="40" <?php if ($rcek1['wash_temp'] == '40') {
													echo "checked";
												} ?>>
											40&deg;C
										</label>
										<label><input type="checkbox" name="wash_temp" id="wash_temp" class="minimal"
												value="60" <?php if ($rcek1['wash_temp'] == '60') {
													echo "checked";
												} ?>>
											60&deg;C
										</label>
									...
									<div class="col-sm-2">
										<input name="wash_colorchange" type="text" class="form-control"
											id="wash_colorchange" value="<?php echo $rcek1['wash_colorchange']; ?>"
								...
								<div class="form-group" id="stat_wf" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_wf" class="form-control select2" id="stat_wf"
											onChange="tampil2();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_wf'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="diswf" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="dwash_colorchange" type="text" class="form-control"
											id="dwash_colorchange" value="<?php echo $rcekD['dwash_colorchange']; ?>"
											placeholder="4-5 Color Change">
									...
									<div class="col-sm-2">
										...
										<input name="dwash_staining" type="text" class="form-control" id="dwash_staining"
											value="<?php echo $rcekD['dwash_staining']; ?>"
											placeholder="4-5 Cross Staining">
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="marwf" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="mwash_colorchange" type="text" class="form-control"
											id="mwash_colorchange" value="<?php echo $rcekM['mwash_colorchange']; ?>"
											placeholder="4-5 Color Change">
									...
									<div class="col-sm-2">
										...
										<input name="mwash_staining" type="text" class="form-control" id="mwash_staining"
											value="<?php echo $rcekM['mwash_staining']; ?>"
											placeholder="4-5 Cross Staining">
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="ranwf" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="rwash_colorchange" type="text" class="form-control"
											id="rwash_colorchange" value="<?php echo $rcekR['rwash_colorchange']; ?>"
								...
								<div class="form-group" id="c2" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="water_colorchange" type="text" class="form-control"
											id="water_colorchange" value="<?php echo $rcek1['water_colorchange']; ?>"
											placeholder="4-5 Color Change">
								...
								<div class="form-group" id="stat_wtr" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_wtr" class="form-control select2" id="stat_wtr"
											onChange="tampil2();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_wtr'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="ranwtr" style="display:none;">
									...
									<div class="col-sm-2">
										...
										<input name="rwater_acetate" type="text" class="form-control" id="rwater_acetate"
											value="<?php echo $rcekR['rwater_acetate']; ?>" placeholder="4 Acetate"
											readonly>
									</div>
									<div class="col-sm-2">
									...
									<div class="col-sm-2">
										...
										<input name="rwater_acrylic" type="text" class="form-control" id="rwater_acrylic"
											value="<?php echo $rcekR['rwater_acrylic']; ?>" placeholder="4 Acrylic"
											readonly>
									</div>
									<div class="col-sm-2">
										...
										<input name="rwater_staining" type="text" class="form-control" id="rwater_staining"
											value="<?php echo $rcekR['rwater_staining']; ?>" placeholder="S.Staining"
											readonly>
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="c3" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="acid_colorchange" type="text" class="form-control"
											id="acid_colorchange" value="<?php echo $rcek1['acid_colorchange']; ?>"
											placeholder="4-5 Color Change">
								...
								<div class="form-group" id="stat_pac" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_pac" class="form-control select2" id="stat_pac"
											onChange="tampil2();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_pac'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="dispac" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="dacid_colorchange" type="text" class="form-control"
											id="dacid_colorchange" value="<?php echo $rcekD['dacid_colorchange']; ?>"
											placeholder="4-5 Color Change">
								...
								<div class="form-group" id="marpac" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="macid_colorchange" type="text" class="form-control"
											id="macid_colorchange" value="<?php echo $rcekM['macid_colorchange']; ?>"
											placeholder="4-5 Color Change">
								...
								<div class="form-group" id="ranpac" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="racid_colorchange" type="text" class="form-control"
											id="racid_colorchange" value="<?php echo $rcekR['racid_colorchange']; ?>"
											placeholder="4-5 Color Change" readonly>
									...
									<div class="col-sm-2">
										...
										<input name="racid_staining" type="text" class="form-control" id="racid_staining"
											value="<?php echo $rcekR['racid_staining']; ?>" placeholder="S.Staining"
											readonly>
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="c4" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="alkaline_colorchange" type="text" class="form-control"
											...
											placeholder="4-5 Color Change">
										<input name="alkaline_acetate" type="text" class="form-control"
											id="alkaline_acetate" value="<?php echo $rcek1['alkaline_acetate']; ?>"
											placeholder="4 Acetate">
									</div>
									<div class="col-sm-2">
									...
									<div class="col-sm-2">
										...
										<input name="alkaline_acrylic" type="text" class="form-control"
											id="alkaline_acrylic" value="<?php echo $rcek1['alkaline_acrylic']; ?>"
											placeholder="4 Acrylic">
									</div>
									<div class="col-sm-2">
										...
										<input name="alkaline_staining" type="text" class="form-control"
											id="alkaline_staining" value="<?php echo $rcek1['alkaline_staining']; ?>"
								...
								<div class="form-group" id="stat_pal" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_pal" class="form-control select2" id="stat_pal"
											onChange="tampil2();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_pal'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="dispal" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="dalkaline_colorchange" type="text" class="form-control"
											id="dalkaline_colorchange"
											value="<?php echo $rcekD['dalkaline_colorchange']; ?>"
											placeholder="4-5 Color Change">
										<input name="dalkaline_acetate" type="text" class="form-control"
											id="dalkaline_acetate" value="<?php echo $rcekD['dalkaline_acetate']; ?>"
											placeholder="4 Acetate">
									</div>
									<div class="col-sm-2">
										<input name="dalkaline_cotton" type="text" class="form-control"
											id="dalkaline_cotton" value="<?php echo $rcekD['dalkaline_cotton']; ?>"
											placeholder="4 Cotton">
									...
									<div class="col-sm-2">
										...
										<input name="dalkaline_acrylic" type="text" class="form-control"
											id="dalkaline_acrylic" value="<?php echo $rcekD['dalkaline_acrylic']; ?>"
											placeholder="4 Acrylic">
									</div>
									<div class="col-sm-2">
								...
								<div class="form-group" id="marpal" style="display:none;">
									<label for="marpal" class="col-sm-2 control-label">PERSPIRATION ALKALINE
										(MARGINAL)</label>
									<div class="col-sm-2">
										<input name="malkaline_colorchange" type="text" class="form-control"
											id="malkaline_colorchange"
											value="<?php echo $rcekM['malkaline_colorchange']; ?>"
											placeholder="4-5 Color Change">
										<input name="malkaline_acetate" type="text" class="form-control"
											id="malkaline_acetate" value="<?php echo $rcekM['malkaline_acetate']; ?>"
											placeholder="4 Acetate">
									</div>
									<div class="col-sm-2">
										<input name="malkaline_cotton" type="text" class="form-control"
											id="malkaline_cotton" value="<?php echo $rcekM['malkaline_cotton']; ?>"
											placeholder="4 Cotton">
									...
									<div class="col-sm-2">
										...
										<input name="malkaline_acrylic" type="text" class="form-control"
											id="malkaline_acrylic" value="<?php echo $rcekM['malkaline_acrylic']; ?>"
											placeholder="4 Acrylic">
									</div>
									<div class="col-sm-2">
								...
								<div class="form-group" id="ranpal" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="ralkaline_colorchange" type="text" class="form-control"
											id="ralkaline_colorchange"
											value="<?php echo $rcekR['ralkaline_colorchange']; ?>"
											placeholder="4-5 Color Change" readonly>
										<input name="ralkaline_acetate" type="text" class="form-control"
											id="ralkaline_acetate" value="<?php echo $rcekR['ralkaline_acetate']; ?>"
											placeholder="4 Acetate" readonly>
									</div>
									<div class="col-sm-2">
										<input name="ralkaline_cotton" type="text" class="form-control"
											id="ralkaline_cotton" value="<?php echo $rcekR['ralkaline_cotton']; ?>"
											placeholder="4 Cotton" readonly>
									...
									<div class="col-sm-2">
										<input name="ralkaline_poly" type="text" class="form-control" id="ralkaline_poly"
											value="<?php echo $rcekR['ralkaline_poly']; ?>" placeholder="4 Polyester"
											readonly>
										<input name="ralkaline_acrylic" type="text" class="form-control"
											id="ralkaline_acrylic" value="<?php echo $rcekR['ralkaline_acrylic']; ?>"
											placeholder="4 Acrylic" readonly>
									</div>
									<div class="col-sm-2">
								...
								<div class="form-group" id="stat_cr" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_cr" class="form-control select2" id="stat_cr"
											onChange="tampil2();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_cr'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="discr" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dcrock_note"
											maxlength="50"><?php echo $rcekD['dcrock_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="marcr" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="mcrock_note"
											maxlength="50"><?php echo $rcekM['mcrock_note']; ?></textarea>
									</div>
								</div>
								...
								<div class="form-group" id="stat_py" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_py" class="form-control select2" id="stat_py"
											onChange="tampil2();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_py'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="dispy" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="dphenolic_colorchange" type="text" class="form-control"
											id="dphenolic_colorchange"
											value="<?php echo $rcekD['dphenolic_colorchange']; ?>"
											placeholder="4-5 Color Change">
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="marpy" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="mphenolic_colorchange" type="text" class="form-control"
											id="mphenolic_colorchange"
											value="<?php echo $rcekM['mphenolic_colorchange']; ?>"
											placeholder="4-5 Color Change">
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="ranpy" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="rphenolic_colorchange" type="text" class="form-control"
											id="rphenolic_colorchange"
											value="<?php echo $rcekR['rphenolic_colorchange']; ?>"
											placeholder="4-5 Color Change" readonly>
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="c7" style="display:none;">
									<label for="cm_printing" class="col-sm-2 control-label">COLOR MIGRATION - OVEN
										TEST</label>
									<div class="col-sm-2">
								...
								<div class="form-group" id="stat_cmo" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_cmo" class="form-control select2" id="stat_cmo"
											onChange="tampil2();" style="width: 100%;">
								...
								<div class="form-group" id="discmo" style="display:none;">
									<label for="discmo" class="col-sm-2 control-label">COLOR MIGRATION - OVEN TEST
										(DIS)</label>
									<div class="col-sm-2">
								...
								<div class="form-group" id="rancmo" style="display:none;">
									<label for="rancmo" class="col-sm-2 control-label">COLOR MIGRATION - OVEN TEST
										(RAN)</label>
									<div class="col-sm-2">
										...
										<table width=100% class="sc">
											...
											<tr>
												...
												<td><input class="form-control" type="text" name="sco_acid_original1"
														value="<?= $sco_acid_original[0]; ?>" placeholder="Original 1 Min">
												</td>
												<td><input class="form-control" type="text" name="sco_acid_original2"
														value="<?= $sco_acid_original[1]; ?>" placeholder="Original 2 Min">
												</td>
												<td><input class="form-control" type="text" name="sco_acid_original3"
														value="<?= $sco_acid_original[2]; ?>" placeholder="Original 3 Min">
												</td>
												...
												<td rowspan=4 valign="top">
													<textarea class="form-control"
														placeholder="Note harus diakhir tanda titik" name="sc_note"
														maxlength="50" rows="7"><?php echo $rcek1['sc_note']; ?></textarea>
											...
											<tr>
												...
												<td><input class="form-control" type="text" name="sca_acid_original1"
														value="<?= $sca_acid_original[0] ?>" placeholder="Original 1 min">
												</td>
												<td><input class="form-control" type="text" name="sca_acid_original2"
														value="<?= $sca_acid_original[1] ?>" placeholder="Original 2 min">
												</td>
												<td><input class="form-control" type="text" name="sca_acid_original3"
														value="<?= $sca_acid_original[2] ?>" placeholder="Original 3 min">
												</td>
												<td>
											...
											<tr>
												...
												<td><input class="form-control" type="text" name="sco_alkaline_afterwash1"
														value="<?= $sco_alkaline_afterwash[0] ?>"
														placeholder="Afterwash 1 min"></td>
												<td><input class="form-control" type="text" name="sco_alkaline_afterwash2"
														value="<?= $sco_alkaline_afterwash[1] ?>"
														placeholder="Afterwash 2 min"></td>
												<td><input class="form-control" type="text" name="sco_alkaline_afterwash3"
														value="<?= $sco_alkaline_afterwash[2] ?>"
														placeholder="Afterwash 3 min"></td>
												<td>
								...
								<div class="form-group" id="discm" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dcm_dye_note"
											maxlength="50"><?php echo $rcekD['dcm_dye_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="rancm" style="display:none;">
									...
									<div class="col-sm-2">
										<label><input type="checkbox" name="rcm_dye_temp" id="rcm_dye_temp" class="minimal"
												...
												readonly> 90&deg;C x 24h &nbsp; &nbsp; &nbsp; &nbsp;
										</label>
										<label><input type="checkbox" name="rcm_dye_temp" id="rcm_dye_temp" class="minimal"
												value="70" <?php if ($rcekR['rcm_dye_temp'] == '70') {
													echo "checked";
												} ?>
												readonly> 70&deg;C x 48h
										</label>
									</div>
									<div class="col-sm-2">
								...
								<div class="form-group" id="stat_lg" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_lg" class="form-control select2" id="stat_lg"
											onChange="tampil2();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_lg'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="c10" style="display:none;">
									<label for="light_pers" class="col-sm-2 control-label">LIGHT PERSPIRATION
										FASTNESS</label>
									<div class="col-sm-2">
										<input name="light_pers_colorchange" type="text" class="form-control"
											id="light_pers_colorchange"
											value="<?php echo $rcek1['light_pers_colorchange']; ?>"
								...
								<div class="form-group" id="stat_lp" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_lp" class="form-control select2" id="stat_lp"
											onChange="tampil2();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_lp'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="dislp" style="display:none;">
									<label for="dislp" class="col-sm-2 control-label">LIGHT PERSPIRATION FASTNESS
										(DIS)</label>
									<div class="col-sm-2">
								...
								<div class="form-group" id="ranlp" style="display:none;">
									<label for="ranlp" class="col-sm-2 control-label">LIGHT PERSPIRATION FASTNESS
										(RAN)</label>
									<div class="col-sm-2">
								...
								<div class="form-group" id="c11" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="saliva_staining" type="text" class="form-control" id="saliva_staining"
											value="<?php echo $rcek1['saliva_staining']; ?>"
											placeholder="4-5 Color Staining">
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="stat_slv" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_slv" class="form-control select2" id="stat_slv"
											onChange="tampil2();" style="width: 100%;">
								...
								<div class="form-group" id="disslv" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="dsaliva_staining" type="text" class="form-control"
											id="dsaliva_staining" value="<?php echo $rcekD['dsaliva_staining']; ?>"
											placeholder="4-5 Color Staining">
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="ranslv" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="rsaliva_staining" type="text" class="form-control"
											id="rsaliva_staining" value="<?php echo $rcekR['rsaliva_staining']; ?>"
											placeholder="4-5 Color Staining" readonly>
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="stat_bld" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_bld" class="form-control select2" id="stat_bld"
											onChange="tampil2();" style="width: 100%;">
								...
								<div class="form-group" id="ranbld" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="rbleeding" type="text" class="form-control" id="rbleeding"
											value="<?php echo $rcekR['rbleeding']; ?>" placeholder="Color Staining"
											readonly>
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="stat_chl" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_chl" class="form-control select2" id="stat_chl"
											onChange="tampil2();" style="width: 100%;">
								...
								<div class="form-group" id="c15" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="dye_tf_sstaining" type="text" class="form-control"
											id="dye_tf_sstaining" value="<?php echo $rcek1['dye_tf_sstaining']; ?>"
											placeholder="Self Staining">
										<input name="dye_tf_cstaining" type="text" class="form-control"
											id="dye_tf_cstaining" value="<?php echo $rcek1['dye_tf_cstaining']; ?>"
											placeholder="Color Staining">
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="stat_dye" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_dye" class="form-control select2" id="stat_dye"
											onChange="tampil2();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_dye'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="disdye" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="ddye_tf_cstaining" type="text" class="form-control"
											id="ddye_tf_cstaining" value="<?php echo $rcekD['ddye_tf_cstaining']; ?>"
											placeholder="Color Staining">
										<input name="ddye_tf_sstaining" type="text" class="form-control"
											id="ddye_tf_sstaining" value="<?php echo $rcekD['ddye_tf_sstaining']; ?>"
											placeholder="Self Staining">
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="mardye" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="mdye_tf_cstaining" type="text" class="form-control"
											id="mdye_tf_cstaining" value="<?php echo $rcekM['mdye_tf_cstaining']; ?>"
											placeholder="Color Staining">
										<input name="mdye_tf_sstaining" type="text" class="form-control"
											id="mdye_tf_sstaining" value="<?php echo $rcekM['mdye_tf_sstaining']; ?>"
											placeholder="Self Staining">
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
								...
								<div class="form-group" id="randye" style="display:none;">
									...
									<div class="col-sm-2">
										<input name="rdye_tf_cstaining" type="text" class="form-control"
											id="rdye_tf_cstaining" value="<?php echo $rcekR['rdye_tf_cstaining']; ?>"
											placeholder="Color Staining" readonly>
										<input name="rdye_tf_sstaining" type="text" class="form-control"
											id="rdye_tf_sstaining" value="<?php echo $rcekR['rdye_tf_sstaining']; ?>"
											placeholder="Self Staining" readonly>
									</div>
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
						...
						<div class="tab-pane" id="tab_3">
							<form class="form-horizontal" action="" method="post" enctype="multipart/form-data" name="form3"
								...
								<input name="wick_w3" type="text" class="form-control" id="wick_w3" value="<?php echo $rcek1['wick_w3']; ?>" placeholder="WID 3">
									...
									<div class="col-sm-2">
										<select name="stat_wic" class="form-control select2" id="stat_wic"
											onChange="tampil1();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_wic'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<input name="wick_w3" type="text" class="form-control" id="wick_w3" value="<?php echo $rcek1['wick_w3']; ?>" placeholder="WID 3">
									...
									<div class="col-sm-2">
										<select name="stat_wic2" class="form-control select2" id="stat_wic2"
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_wic2'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<input name="wick_w3" type="text" class="form-control" id="wick_w3" value="<?php echo $rcek1['wick_w3']; ?>" placeholder="WID 3"> -->
									...
									<div class="col-sm-2">
										<select name="stat_wic1" class="form-control select2" id="stat_wic1"
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_wic1'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<input name="wick_w3" type="text" class="form-control" id="wick_w3" value="<?php echo $rcek1['wick_w3']; ?>" placeholder="WID 3"> -->
									...
									<div class="col-sm-2">
										<select name="stat_wic3" class="form-control select2" id="stat_wic3"
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_wic3'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
							...
							<label for="stat_wic" class="col-sm-2 control-label">STATUS</label>
								...
								<input name="absor_f3" type="text" class="form-control" id="absor_f3" value="<?php echo $rcek1['absor_f3']; ?>" placeholder="ORIGINAL 3">
									...
									<div class="col-sm-2">
										<select name="stat_abs" class="form-control select2" id="stat_abs"
											onChange="tampil1();" style="width: 100%;">
								...
								<input name="dabsor_f3" type="text" class="form-control" id="dabsor_f3" value="<?php echo $rcekD['dabsor_f3']; ?>" placeholder="ORIGINAL 3">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dabsor_note"
											maxlength="50"><?php echo $rcekD['dabsor_note']; ?></textarea>
									</div>
								</div>
							...
							<div class="col-sm-2">
								<input name="dry3" type="text" class="form-control" id="dry3" value="<?php echo $rcek1['dry3']; ?>" placeholder="3">
									...
									<div class="col-sm-2">
										<select name="stat_dry" class="form-control select2" id="stat_dry"
											onChange="tampil1();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_dry'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dry_note" maxlength="50"
											rows="1"><?php echo $rcek1['dry_note']; ?></textarea>
									</div>
								</div>
							...
							<div class="col-sm-2">
								...
								<div class="form-group" id="mardry" style="display:none;">
									<label for="mardry" class="col-sm-2 control-label">DRYING TIME ORIGINAL
										(MARGINAL)</label>
									<div class="col-sm-2">
								...
								<input name="dryaf3" type="text" class="form-control" id="dryaf3" value="<?php echo $rcek1['dryaf3']; ?>" placeholder="3">
									...
									<div class="col-sm-2">
										<select name="stat_dry1" class="form-control select2" id="stat_dry1"
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_dry1'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
							...
							<div class="col-sm-2">
								...
								<div class="form-group" id="mardry1" style="display:none;">
									<label for="mardry1" class="col-sm-2 control-label">DRYING TIME AFTERWASH
										(MARGINAL)</label>
									<div class="col-sm-2">
							...
							<div class="col-sm-2">
								<input name="repp4" type="text" class="form-control" id="repp4" value="<?php echo $rcek1['repp4']; ?>" placeholder="4">
									...
									<div class="col-sm-2">
										<select name="stat_wp" class="form-control select2" id="stat_wp"
											onChange="tampil1();" style="width: 100%;">
							...
							<div class="col-sm-2">
								<input name="repp4" type="text" class="form-control" id="repp4" value="<?php echo $rcek1['repp4']; ?>" placeholder="4">
									...
									<div class="col-sm-2">
										<select name="stat_wp1" class="form-control select2" id="stat_wp1"
											onChange="tampil1();" style="width: 100%;">
							...
							<label for="stat_wp" class="col-sm-2 control-label">STATUS</label>
								...
								<div class="form-group" id="diswp1" style="display:none;">
									<label for="diswp1" class="col-sm-2 control-label">WATER REPPELENT AFTERWASH
										(DIS)</label>
									<div class="col-sm-2">
							...
							<div class="col-sm-2">
								...
								<div class="form-group" id="ranwp1" style="display:none;">
									<label for="ranwp1" class="col-sm-2 control-label">WATER REPPELENT AFTERWASH
										(RAN)</label>
									<div class="col-sm-2">
							...
							<div class="col-sm-2">
								...
								<div class="form-group" id="f5" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="ph_note" maxlength="50"
											rows="1"><?php echo $rcek1['ph_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="stat_ph" style="display:none;">
									<label for="stat_ph" class="col-sm-2 control-label">STATUS</label>
									<div class="col-sm-2">
										<select name="stat_ph" class="form-control select2" id="stat_ph"
											onChange="tampil1();" style="width: 100%;">
											...
											; ?>value="PASS">PASS</option>
											<option <?php if ($rcek1['stat_ph'] == "MARGINAL PASS") { ?> selected=selected
												<?php }
											; ?>value="MARGINAL PASS">MARGINAL PASS</option>
								...
								<div class="form-group" id="disph" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="dph_note" maxlength="50"
											rows="1"><?php echo $rcekD['dph_note']; ?></textarea>
									</div>
								</div>
								<div class="form-group" id="marph" style="display:none;">
									...
									<div class="col-sm-2">
										<textarea class="form-control" placeholder="Note harus diakhir tanda titik"
											name="mph_note" maxlength="50"
											rows="1"><?php echo $rcekM['mph_note']; ?></textarea>
									</div>
								</div>
								...
								<div class="form-group" id="stat_sor" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_sor" class="form-control select2" id="stat_sor"
											onChange="tampil1();" style="width: 100%;">
								...
								<div class="form-group" id="stat_hum" style="display:none;">
									...
									<div class="col-sm-2">
										<select name="stat_hum" class="form-control select2" id="stat_hum"
											onChange="tampil1();" style="width: 100%;">
...
<?php } ?>
<?php if ($cek1 > 0 and isset($nodemand)) { ?>
			  ...
			  <?php } ?>-->
							...
							<tr>
								...
								<td colspan="6">
									<?php if ($rcek1['stat_fib'] == "RANDOM") {
										...
										<?php if ($rcekR['rfc_poly'] != "") {
											echo $rcekR['rfc_poly'] . "% " . $rcekR['rfc_poly1'];
										} ?>
										<?php if ($rcekR['rfc_elastane'] != "") {
										...
										<?php if ($rcek1['fc_poly'] != "") {
											echo $rcek1['fc_poly'] . "% " . $rcek1['fc_poly1'];
										} ?>
										<?php if ($rcek1['fc_elastane'] != "") {
						...
						<?php if ($rcek1['recover_l1'] != "" or $rcek1['recover_l2'] != "" or $rcek1['recover_l3'] != "" or $rcek1['recover_l4'] != "" or $rcek1['recover_l5'] != "") { ?>
							<tr>
								...
								<td>
									<?php if ($rcek1['stat_sr'] == "RANDOM") {
										echo $rcekR['rrecover_l11'];
									} else {
										echo $rcek1['recover_l11'];
									} ?>
								</td>
								<td>
									<?php if ($rcek1['stat_sr'] == "RANDOM") {
							...
							<tr>
								...
								<td>
									<?php if ($rcek1['stat_sr'] == "RANDOM") {
										echo $rcekR['rrecover_w11'];
									} else {
										echo $rcek1['recover_w11'];
									} ?>
								</td>
								<td>
									<?php if ($rcek1['stat_sr'] == "RANDOM") {
				...
				<table class="table">
					<?php if ($rcek1['wick_l1'] != "" or $rcek1['wick_l2'] != "" or $rcek1['wick_l3'] != "" or $rcek1['wick_w1'] != "" or $rcek1['wick_w2'] != "" or $rcek1['wick_w3'] != "") { ?>
						<tr>
							...
							<!--<td><?php if ($rcek1['stat_wic'] == "RANDOM") {
								echo $rcekR['rwick_l2'];
							} else {
								echo $rcek1['wick_l2'];
							} ?></td>
				...
				<td><?php if ($rcek1['stat_wic'] == "RANDOM") {
					...
					<?php if ($rcek1['absor_f1'] != "" or $rcek1['absor_f2'] != "" or $rcek1['absor_f3'] != "" or $rcek1['absor_b1'] != "" or $rcek1['absor_b2'] != "" or $rcek1['absor_b3'] != "") { ?>
						<tr>
							...
							<!--<td><?php if ($rcek1['stat_abs'] == "RANDOM") {
						...
						<tr>
							...
							<!--<td><?php if ($rcek1['stat_abs1'] == "RANDOM") {
					...
					<?php if ($rcek1['repp1'] != "" or $rcek1['repp2'] != "") { ?>
						<tr>
							...
							<!--<td><?php if ($rcek1['stat_wp'] == "RANDOM") {
								echo $rcekR['rrepp2'];
							} else {
								echo $rcek1['repp2'];
							} ?></td>
...
<?php
	...
	if ($_POST['physical_save'] == "save") {
		...
		if ($tq_test_2_array) {
			if ($_POST['spirality_status'] == '0') {
				$sqlPHY = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"UPDATE db_qc.tbl_tq_test_2 SET spirality_status = NULL WHERE id_nokk = ?",
					[$id_tq_test_2]
				);
			} else {
				$sqlPHY = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"UPDATE db_qc.tbl_tq_test_2 SET spirality_status = ? WHERE id_nokk = ?",
					[$spirality_status, $id_tq_test_2]
				);
			}
		} else {
			if ($_POST['spirality_status'] != '0') {
				$array_insert[] = 1;
				$sql_no_demand = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"INSERT INTO db_qc.tbl_tq_test_2 (id_nokk, spirality_status) VALUES (?, ?)",
					[$id_tq_test_2, $spirality_status]
				);
			}
		}
	}
	...
	if ($_POST['colorfastness_save'] == "save") {  // bleeding_root save
		...
		if ($tq_test_2_array or count($array_insert) > 0) {
			if ($bleeding_root == '') {
				$sqlPHY = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"UPDATE db_qc.tbl_tq_test_2 SET bleeding_root = NULL WHERE id_nokk = ?",
					[$id_tq_test_2]
				);
			} else {
				$sqlPHY = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"UPDATE db_qc.tbl_tq_test_2 SET bleeding_root = ? WHERE id_nokk = ?",
					[$bleeding_root, $id_tq_test_2]
				);
			}
		} else {
			if ($_POST['bleeding_root'] != '0') { //insert 
				$sql_no_demand = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"INSERT INTO db_qc.tbl_tq_test_2 (id_nokk, bleeding_root) VALUES (?, ?)",
					[$id_tq_test_2, $bleeding_root]
				);
			}
		}

		// konsep: kalau sudah ada record -> UPDATE, kalau belum ada -> INSERT
		$cek_disptest2 = sqlsrv_query(
			$con_db_qc_sqlsrv,
			"SELECT TOP 1 id_nokk FROM db_qc.tbl_tq_disptest_2 WHERE id_nokk = ?",
			[$id_tq_test_2],
			["Scrollable" => SQLSRV_CURSOR_KEYSET]
		);

		$ada_disptest2 = 0;
		if ($cek_disptest2 !== false) {
			$ada_disptest2 = (sqlsrv_fetch_array($cek_disptest2, SQLSRV_FETCH_ASSOC)) ? 1 : 0;
		}
		...
		if ($dbleeding_root != '') {
			if ($ada_disptest2 > 0) {
				$sqlPHY = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"UPDATE db_qc.tbl_tq_disptest_2 SET dbleeding_root = ? WHERE id_nokk = ?",
					[$dbleeding_root, $id_tq_test_2]
				);
			} else {
				$sql_no_demand = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"INSERT INTO db_qc.tbl_tq_disptest_2 (id_nokk, dbleeding_root) VALUES (?, ?)",
					[$id_tq_test_2, $dbleeding_root]
				);
			}
		} else {
			if ($ada_disptest2 > 0) {
				$sqlPHY = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"UPDATE db_qc.tbl_tq_disptest_2 SET dbleeding_root = NULL WHERE id_nokk = ?",
					[$id_tq_test_2]
				);
			} else {
				$sql_no_demand = sqlsrv_query(
					$con_db_qc_sqlsrv,
					"INSERT INTO db_qc.tbl_tq_disptest_2 (id_nokk, dbleeding_root) VALUES (?, NULL)",
					[$id_tq_test_2]
				);
			}
		}
	...
	if ($_POST['physical_save'] == "save") {
		...
		foreach ($fields as $field) {
			...
			if ($value != "0" && $value != "") {
				...
				$selectSql = "SELECT * FROM db_qc.tbl_tq_test_2 WHERE id_nokk = '$id_tq_test_2'";
				$result = sqlsrv_query($con_db_qc_sqlsrv, $selectSql, array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));

				if ($result === false) {
					echo "<pre>SQL ERROR selectSql:\n";
					print_r(sqlsrv_errors());
					echo "</pre>";
					die;
				}
				...
				if (sqlsrv_num_rows($result) > 0) {
					...
					$updateSql = "UPDATE db_qc.tbl_tq_test_2 SET $field = '$value' WHERE id_nokk = '$id_tq_test_2'";
					$sqlPHY = sqlsrv_query($con_db_qc_sqlsrv, $updateSql);
				} else {
					// Data belum ada, lakukan INSERT
					$insertSql = "INSERT INTO db_qc.tbl_tq_test_2 (id_nokk, $field) VALUES ('$id_tq_test_2','$value')";
					$sqlPHY = sqlsrv_query($con_db_qc_sqlsrv, $insertSql);
				}

				if ($sqlPHY === false) {
					echo "<pre>SQL ERROR update/insert:\n";
					print_r(sqlsrv_errors());
					echo "</pre>";
					die;
				}
				...
				$sqlPHY = sqlsrv_query($con_db_qc_sqlsrv, "UPDATE db_qc.tbl_tq_test_2 SET $field = null WHERE id_nokk = '$id_tq_test_2'");

				if ($sqlPHY === false) {
					echo "<pre>SQL ERROR set null:\n";
					print_r(sqlsrv_errors());
					echo "</pre>";
					die;
				}
...
<?php
	if ($_POST['physical_save'] == "save" and $cek1 > 0) {
		...
		if ($sqlPHY) {
			...
			$sqlPHYDI = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tq_disptest (
					id_nokk,
					...
					dh_shrinkage_temp, dh_shrinkage_l1, dh_shrinkage_w1, dh_shrinkage_grd, dh_shrinkage_app, dh_shrinkage_note,
					dss_temp, dss_washes3, dss_washes10, dss_washes15, dss_cmt,
					dshrinkage_l1, dshrinkage_l2, dshrinkage_l3, dshrinkage_l4, dshrinkage_l5, dshrinkage_l6,
					dshrinkage_w1, dshrinkage_w2, dshrinkage_w3, dshrinkage_w4, dshrinkage_w5, dshrinkage_w6,
					dspirality1, dspirality2, dspirality3, dspirality4, dspirality5, dspirality6,
					dss_linedry, dss_tumbledry, dsns_note,
					dapperss_ch1, dapperss_ch2, dapperss_ch3, dapperss_ch4,
					dapperss_cc1, dapperss_cc2, dapperss_cc3, dapperss_cc4,
					dapperss_st,
					dapperss_pf1, dapperss_pf2, dapperss_pf3, dapperss_pf4,
					dapperss_pb1, dapperss_pb2, dapperss_pb3, dapperss_pb4,
					dapperss_sf1, dapperss_sf2, dapperss_sf3, dapperss_sf4,
					dapperss_sb1, dapperss_sb2, dapperss_sb3, dapperss_sb4,
					dapperss_note,
					dpm_f1, dpm_b1, dpm_f2, dpm_b2, dpm_f3, dpm_b3, dpm_f4, dpm_b4, dpm_f5, dpm_b5, dpillm_note,
					dpl_f1, dpl_b1, dpl_f2, dpl_b2, dpl_f3, dpl_b3, dpl_f4, dpl_b4, dpl_f5, dpl_b5, dpilll_note,
					dpb_f1, dpb_b1, dpb_f2, dpb_b2, dpb_f3, dpb_b3, dpb_f4, dpb_b4, dpb_f5, dpb_b5, dpillb_note,
					dprt_f1, dprt_b1, dprt_f2, dprt_b2, dprt_f3, dprt_b3, dprt_f4, dprt_b4, dprt_f5, dprt_b5, dpillr_note,
					dabration, dabr_note,
					dsm_l1, dsm_w1, dsm_l2, dsm_w2, dsm_l3, dsm_w3, dsm_l4, dsm_w4, dsnam_note,
					dsp_grdl1, dsp_clsl1, dsp_shol1, dsp_medl1, dsp_lonl1,
					dsp_grdw1, dsp_clsw1, dsp_show1, dsp_medw1, dsp_lonw1,
					dsp_grdl2, dsp_clsl2, dsp_shol2, dsp_medl2, dsp_lonl2,
					dsp_grdw2, dsp_clsw2, dsp_show2, dsp_medw2, dsp_lonw2,
					dsnap_note,
					dsb_l1, dsb_w1, dsb_l2, dsb_w2, dsb_l3, dsb_w3, dsb_l4, dsb_w4, dsnab_note,
					dbs_instron, dbs_mullen, dbs_tru, dbs_tru2, dburs_note,
					dthick1, dthick2, dthick3, dthickav, dthick_note,
					dstretch_l1, dstretch_w1, dstretch_l2, dstretch_w2, dstretch_l3, dstretch_w3, dstretch_l4, dstretch_w4,
					dstretch_l5, dstretch_w5, dload_stretch, dstretch_note,
					drecover_l1, drecover_w1, drecover_l2, drecover_w2, drecover_l3, drecover_w3, drecover_l4, drecover_w4,
					drecover_l5, drecover_w5,
					-- NOTE: di potongan kamu ada drecover_w11 tapi TIDAK ada drecover_l11 (mungkin kepotong)
					drecover_w11, drecover_l21, drecover_w21, drecover_l31, drecover_w31, drecover_l41, drecover_w41, drecover_l51, drecover_w51,
					drecover_note,
					dgrowth_l1, dgrowth_w1, dgrowth_l2, dgrowth_w2,
					drec_growth_l1, drec_growth_w1, drec_growth_l2, drec_growth_w2,
					dgrowth_note,
					dapper_ch1, dapper_ch2, dapper_ch3,
					dapper_cc1, dapper_cc2, dapper_cc3,
					dapper_st, dapper_st2, dapper_st3,
					dapper_pf1, dapper_pf2, dapper_pf3,
					dapper_pb1, dapper_pb2, dapper_pb3,
					dapper_acetate, dapper_cotton, dapper_nylon, dapper_poly, dapper_acrylic, dapper_wool,
					dapper_note,
				...
				VALUES (
					...
					'$_POST[dh_shrinkage_temp]', '$_POST[dh_shrinkage_l1]', '$_POST[dh_shrinkage_w1]', '$_POST[dh_shrinkage_grd]', '$_POST[dh_shrinkage_app]', '$_POST[dh_shrinkage_note]',
					'$_POST[dss_temp]', '$_POST[dss_washes3]', '$_POST[dss_washes10]', '$_POST[dss_washes15]', '$_POST[dss_cmt]',
					'$_POST[dshrinkage_len1]', '$_POST[dshrinkage_len2]', '$_POST[dshrinkage_len3]', '$_POST[dshrinkage_len4]', '$_POST[dshrinkage_len5]', '$_POST[dshrinkage_len6]',
					'$_POST[dshrinkage_wid1]', '$_POST[dshrinkage_wid2]', '$_POST[dshrinkage_wid3]', '$_POST[dshrinkage_wid4]', '$_POST[dshrinkage_wid5]', '$_POST[dshrinkage_wid6]',
					'$_POST[dspirality1]', '$_POST[dspirality2]', '$_POST[dspirality3]', '$_POST[dspirality4]', '$_POST[dspirality5]', '$_POST[dspirality6]',
					'$_POST[dss_linedry]', '$_POST[dss_tumbledry]', '$_POST[dsns_note]',
					'$_POST[dapperss_ch1]', '$_POST[dapperss_ch2]', '$_POST[dapperss_ch3]', '$_POST[dapperss_ch4]',
					'$_POST[dapperss_cc1]', '$_POST[dapperss_cc2]', '$_POST[dapperss_cc3]', '$_POST[dapperss_cc4]',
					'$_POST[dapperss_st]',
					'$_POST[dapperss_pf1]', '$_POST[dapperss_pf2]', '$_POST[dapperss_pf3]', '$_POST[dapperss_pf4]',
					'$_POST[dapperss_pb1]', '$_POST[dapperss_pb2]', '$_POST[dapperss_pb3]', '$_POST[dapperss_pb4]',
					'$_POST[dapperss_sf1]', '$_POST[dapperss_sf2]', '$_POST[dapperss_sf3]', '$_POST[dapperss_sf4]',
					'$_POST[dapperss_sb1]', '$_POST[dapperss_sb2]', '$_POST[dapperss_sb3]', '$_POST[dapperss_sb4]',
					'$_POST[dapperss_note]',
					'$_POST[dpillingm_f1]', '$_POST[dpillingm_b1]', '$_POST[dpillingm_f2]', '$_POST[dpillingm_b2]', '$_POST[dpillingm_f3]', '$_POST[dpillingm_b3]', '$_POST[dpillingm_f4]', '$_POST[dpillingm_b4]', '$_POST[dpillingm_f5]', '$_POST[dpillingm_b5]', '$_POST[dpillm_note]',
					'$_POST[dpillingl_f1]', '$_POST[dpillingl_b1]', '$_POST[dpillingl_f2]', '$_POST[dpillingl_b2]', '$_POST[dpillingl_f3]', '$_POST[dpillingl_b3]', '$_POST[dpillingl_f4]', '$_POST[dpillingl_b4]', '$_POST[dpillingl_f5]', '$_POST[dpillingl_b5]', '$_POST[dpilll_note]',
					'$_POST[dpillingb_f1]', '$_POST[dpillingb_b1]', '$_POST[dpillingb_f2]', '$_POST[dpillingb_b2]', '$_POST[dpillingb_f3]', '$_POST[dpillingb_b3]', '$_POST[dpillingb_f4]', '$_POST[dpillingb_b4]', '$_POST[dpillingb_f5]', '$_POST[dpillingb_b5]', '$_POST[dpillb_note]',
					'$_POST[dpillingrt_f1]', '$_POST[dpillingrt_b1]', '$_POST[dpillingrt_f2]', '$_POST[dpillingrt_b2]', '$_POST[dpillingrt_f3]', '$_POST[dpillingrt_b3]', '$_POST[dpillingrt_f4]', '$_POST[dpillingrt_b4]', '$_POST[dpillingrt_f5]', '$_POST[dpillingrt_b5]', '$_POST[dpillr_note]',
					'$_POST[dabr]', '$_POST[dabr_note]',
					'$_POST[dsnaggingm_l1]', '$_POST[dsnaggingm_w1]', '$_POST[dsnaggingm_l2]', '$_POST[dsnaggingm_w2]', '$_POST[dsnaggingm_l3]', '$_POST[dsnaggingm_w3]', '$_POST[dsnaggingm_l4]', '$_POST[dsnaggingm_w4]', '$_POST[dsnam_note]',
					'$_POST[dsp_grdl1]', '$_POST[dsp_clsl1]', '$_POST[dsp_shol1]', '$_POST[dsp_medl1]', '$_POST[dsp_lonl1]',
					'$_POST[dsp_grdw1]', '$_POST[dsp_clsw1]', '$_POST[dsp_show1]', '$_POST[dsp_medw1]', '$_POST[dsp_lonw1]',
					'$_POST[dsp_grdl2]', '$_POST[dsp_clsl2]', '$_POST[dsp_shol2]', '$_POST[dsp_medl2]', '$_POST[dsp_lonl2]',
					'$_POST[dsp_grdw2]', '$_POST[dsp_clsw2]', '$_POST[dsp_show2]', '$_POST[dsp_medw2]', '$_POST[dsp_lonw2]',
					'$_POST[dsnap_note]',
					'$_POST[dsnaggingb_l1]', '$_POST[dsnaggingb_w1]', '$_POST[dsnaggingb_l2]', '$_POST[dsnaggingb_w2]', '$_POST[dsnaggingb_l3]', '$_POST[dsnaggingb_w3]', '$_POST[dsnaggingb_l4]', '$_POST[dsnaggingb_w4]', '$_POST[dsnab_note]',
					'$_POST[dinstron]', '$_POST[dmullen]', '$_POST[dtru_burst]', '$_POST[dtru_burst2]', '$_POST[dburs_note]',
					'$_POST[dthick1]', '$_POST[dthick2]', '$_POST[dthick3]', '$_POST[dthickav]', '$_POST[dthick_note]',
					'$_POST[dstretch_l1]', '$_POST[dstretch_w1]', '$_POST[dstretch_l2]', '$_POST[dstretch_w2]', '$_POST[dstretch_l3]', '$_POST[dstretch_w3]', '$_POST[dstretch_l4]', '$_POST[dstretch_w4]',
					'$_POST[dstretch_l5]', '$_POST[dstretch_w5]', '$_POST[dload_stretch]', '$_POST[dstretch_note]',
					'$_POST[drecover_l1]', '$_POST[drecover_w1]', '$_POST[drecover_l2]', '$_POST[drecover_w2]', '$_POST[drecover_l3]', '$_POST[drecover_w3]', '$_POST[drecover_l4]', '$_POST[drecover_w4]',
					'$_POST[drecover_l5]', '$_POST[drecover_w5]',
					'$_POST[drecover_w11]', '$_POST[drecover_l21]', '$_POST[drecover_w21]', '$_POST[drecover_l31]', '$_POST[drecover_w31]', '$_POST[drecover_l41]', '$_POST[drecover_w41]', '$_POST[drecover_l51]', '$_POST[drecover_w51]',
					'$_POST[drecover_note]',
					'$_POST[dgrowth_l1]', '$_POST[dgrowth_w1]', '$_POST[dgrowth_l2]', '$_POST[dgrowth_w2]',
					'$_POST[drec_growth_l1]', '$_POST[drec_growth_w1]', '$_POST[drec_growth_l2]', '$_POST[drec_growth_w2]',
					'$_POST[dgrowth_note]',
					'$_POST[dapper_ch1]', '$_POST[dapper_ch2]', '$_POST[dapper_ch3]',
					'$_POST[dapper_cc1]', '$_POST[dapper_cc2]', '$_POST[dapper_cc3]',
					'$_POST[dapper_st]', '$_POST[dapper_st2]', '$_POST[dapper_st3]',
					'$_POST[dapper_pf1]', '$_POST[dapper_pf2]', '$_POST[dapper_pf3]',
					'$_POST[dapper_pb1]', '$_POST[dapper_pb2]', '$_POST[dapper_pb3]',
					'$_POST[dapper_acetate]', '$_POST[dapper_cotton]', '$_POST[dapper_nylon]', '$_POST[dapper_poly]', '$_POST[dapper_acrylic]', '$_POST[dapper_wool]',
					'$_POST[dapper_note]',
			...
			$sqlPHYM = sqlsrv_query($con_db_qc_sqlsrv, " UPDATE db_qc.tbl_tq_marginal SET
			...
			$sqlPHYMI = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tq_marginal (
					id_nokk,
					mflamability, mfla_note,
					mfc_cott, mfc_poly, mfc_elastane,
					mfc_cott1, mfc_poly1, mfc_elastane1,
					std_mfc_cott1, std_mfc_poly1, std_mfc_elastane1,
					mfibercontent, mfiber_note,
					mfc_wpi, mfc_cpi, mfc_note,
					mf_weight, mfwe_note,
					mf_width, mfwi_note,
					mbow, mskew, mbas_note,
					mh_shrinkage_temp, mh_shrinkage_l1, mh_shrinkage_w1, mh_shrinkage_grd, mh_shrinkage_app, mh_shrinkage_note,
					mss_temp, mss_washes3, mss_washes10, mss_washes15, mss_cmt,
					mshrinkage_l1, mshrinkage_l2, mshrinkage_l3, mshrinkage_l4, mshrinkage_l5, mshrinkage_l6,
					mshrinkage_w1, mshrinkage_w2, mshrinkage_w3, mshrinkage_w4, mshrinkage_w5, mshrinkage_w6,
					mspirality1, mspirality2, mspirality3, mspirality4, mspirality5, mspirality6,
					mss_linedry, mss_tumbledry, msns_note,
					mapperss_ch1, mapperss_ch2, mapperss_ch3, mapperss_ch4,
					mapperss_cc1, mapperss_cc2, mapperss_cc3, mapperss_cc4,
					mapperss_st,
					mapperss_pf1, mapperss_pf2, mapperss_pf3, mapperss_pf4,
					mapperss_pb1, mapperss_pb2, mapperss_pb3, mapperss_pb4,
					mapperss_sf1, mapperss_sf2, mapperss_sf3, mapperss_sf4,
					mapperss_sb1, mapperss_sb2, mapperss_sb3, mapperss_sb4,
					mapperss_note,
					mpm_f1, mpm_b1, mpm_f2, mpm_b2, mpm_f3, mpm_b3, mpm_f4, mpm_b4, mpm_f5, mpm_b5, mpillm_note,
					mpl_f1, mpl_b1, mpl_f2, mpl_b2, mpl_f3, mpl_b3, mpl_f4, mpl_b4, mpl_f5, mpl_b5, mpilll_note,
					mpb_f1, mpb_b1, mpb_f2, mpb_b2, mpb_f3, mpb_b3, mpb_f4, mpb_b4, mpb_f5, mpb_b5, mpillb_note,
					mprt_f1, mprt_b1, mprt_f2, mprt_b2, mprt_f3, mprt_b3, mprt_f4, mprt_b4, mprt_f5, mprt_b5, mpillr_note,
					mabration, mabr_note,
					msm_l1, msm_w1, msm_l2, msm_w2, msm_l3, msm_w3, msm_l4, msm_w4, msnam_note,
					msp_grdl1, msp_clsl1, msp_shol1, msp_medl1, msp_lonl1,
					msp_grdw1, msp_clsw1, msp_show1, msp_medw1, msp_lonw1,
					msp_grdl2, msp_clsl2, msp_shol2, msp_medl2, msp_lonl2,
					msp_grdw2, msp_clsw2, msp_show2, msp_medw2, msp_lonw2,
					msnap_note,
					msb_l1, msb_w1, msb_l2, msb_w2, msb_l3, msb_w3, msb_l4, msb_w4, msnab_note,
					mbs_instron, mbs_mullen, mbs_tru, mbs_tru2, mburs_note,
					mthick1, mthick2, mthick3, mthickav, mthick_note,
					mstretch_l1, mstretch_w1, mstretch_l2, mstretch_w2, mstretch_l3, mstretch_w3, mstretch_l4, mstretch_w4,
					mstretch_l5, mstretch_w5, mload_stretch, mstretch_note,
					mrecover_l1, mrecover_w1, mrecover_l2, mrecover_w2, mrecover_l3, mrecover_w3, mrecover_l4, mrecover_w4,
					mrecover_l5, mrecover_w5,
					mrecover_l11, mrecover_w11, mrecover_l21, mrecover_w21, mrecover_l31, mrecover_w31, mrecover_l41, mrecover_w41, mrecover_l51, mrecover_w51,
					mrecover_note,
					mgrowth_l1, mgrowth_w1, mgrowth_l2, mgrowth_w2,
					mrec_growth_l1, mrec_growth_w1, mrec_growth_l2, mrec_growth_w2,
					mgrowth_note,
					mapper_ch1, mapper_ch2, mapper_ch3,
					mapper_cc1, mapper_cc2, mapper_cc3,
					mapper_st, mapper_st2, mapper_st3,
					mapper_pf1, mapper_pf2, mapper_pf3,
					mapper_pb1, mapper_pb2, mapper_pb3,
					mapper_acetate, mapper_cotton, mapper_nylon, mapper_poly, mapper_acrylic, mapper_wool,
					mapper_note,
					mfibre_transfer, mfibre_grade, mfibre_note,
					modour, modour_note,
					tgl_update
				)
				VALUES (
					'$rcek[id]',
					'$_POST[mflamability]', '$_POST[mfla_note]',
					'$_POST[mfc_cott]', '$_POST[mfc_poly]', '$_POST[mfc_ela]',
					'$_POST[mfc_cott1]', '$_POST[mfc_poly1]', '$_POST[mfc_ela1]',
					'$_POST[std_mfc_cott1]', '$_POST[std_mfc_poly1]', '$_POST[std_mfc_elastane1]',
					'$_POST[mfibercontent]', '$_POST[mfiber_note]',
					'$_POST[mwpi]', '$_POST[mcpi]', '$_POST[mfc_note]',
					'$_POST[mfabric_weight]', '$_POST[mfwe_note]',
					'$_POST[mfabric_width]', '$_POST[mfwi_note]',
					'$_POST[mbow]', '$_POST[mskew]', '$_POST[mbas_note]',
					'$_POST[mh_shrinkage_temp]', '$_POST[mh_shrinkage_l1]', '$_POST[mh_shrinkage_w1]', '$_POST[mh_shrinkage_grd]', '$_POST[mh_shrinkage_app]', '$_POST[mh_shrinkage_note]',
					'$_POST[mss_temp]', '$_POST[mss_washes3]', '$_POST[mss_washes10]', '$_POST[mss_washes15]', '$_POST[mss_cmt]',
					'$_POST[mshrinkage_len1]', '$_POST[mshrinkage_len2]', '$_POST[mshrinkage_len3]', '$_POST[mshrinkage_len4]', '$_POST[mshrinkage_len5]', '$_POST[mshrinkage_len6]',
					'$_POST[mshrinkage_wid1]', '$_POST[mshrinkage_wid2]', '$_POST[mshrinkage_wid3]', '$_POST[mshrinkage_wid4]', '$_POST[mshrinkage_wid5]', '$_POST[mshrinkage_wid6]',
					'$_POST[mspirality1]', '$_POST[mspirality2]', '$_POST[mspirality3]', '$_POST[mspirality4]', '$_POST[mspirality5]', '$_POST[mspirality6]',
					'$_POST[mss_linedry]', '$_POST[mss_tumbledry]', '$_POST[msns_note]',
					'$_POST[mapperss_ch1]', '$_POST[mapperss_ch2]', '$_POST[mapperss_ch3]', '$_POST[mapperss_ch4]',
					'$_POST[mapperss_cc1]', '$_POST[mapperss_cc2]', '$_POST[mapperss_cc3]', '$_POST[mapperss_cc4]',
					'$_POST[mapperss_st]',
					'$_POST[mapperss_pf1]', '$_POST[mapperss_pf2]', '$_POST[mapperss_pf3]', '$_POST[mapperss_pf4]',
					'$_POST[mapperss_pb1]', '$_POST[mapperss_pb2]', '$_POST[mapperss_pb3]', '$_POST[mapperss_pb4]',
					'$_POST[mapperss_sf1]', '$_POST[mapperss_sf2]', '$_POST[mapperss_sf3]', '$_POST[mapperss_sf4]',
					'$_POST[mapperss_sb1]', '$_POST[mapperss_sb2]', '$_POST[mapperss_sb3]', '$_POST[mapperss_sb4]',
					'$_POST[mapperss_note]',
					'$_POST[mpillingm_f1]', '$_POST[mpillingm_b1]', '$_POST[mpillingm_f2]', '$_POST[mpillingm_b2]', '$_POST[mpillingm_f3]', '$_POST[mpillingm_b3]', '$_POST[mpillingm_f4]', '$_POST[mpillingm_b4]', '$_POST[mpillingm_f5]', '$_POST[mpillingm_b5]', '$_POST[mpillm_note]',
					'$_POST[mpillingl_f1]', '$_POST[mpillingl_b1]', '$_POST[mpillingl_f2]', '$_POST[mpillingl_b2]', '$_POST[mpillingl_f3]', '$_POST[mpillingl_b3]', '$_POST[mpillingl_f4]', '$_POST[mpillingl_b4]', '$_POST[mpillingl_f5]', '$_POST[mpillingl_b5]', '$_POST[mpilll_note]',
					'$_POST[mpillingb_f1]', '$_POST[mpillingb_b1]', '$_POST[mpillingb_f2]', '$_POST[mpillingb_b2]', '$_POST[mpillingb_f3]', '$_POST[mpillingb_b3]', '$_POST[mpillingb_f4]', '$_POST[mpillingb_b4]', '$_POST[mpillingb_f5]', '$_POST[mpillingb_b5]', '$_POST[mpillb_note]',
					'$_POST[mpillingrt_f1]', '$_POST[mpillingrt_b1]', '$_POST[mpillingrt_f2]', '$_POST[mpillingrt_b2]', '$_POST[mpillingrt_f3]', '$_POST[mpillingrt_b3]', '$_POST[mpillingrt_f4]', '$_POST[mpillingrt_b4]', '$_POST[mpillingrt_f5]', '$_POST[mpillingrt_b5]', '$_POST[mpillr_note]',
					'$_POST[mabr]', '$_POST[mabr_note]',
					'$_POST[msnaggingm_l1]', '$_POST[msnaggingm_w1]', '$_POST[msnaggingm_l2]', '$_POST[msnaggingm_w2]', '$_POST[msnaggingm_l3]', '$_POST[msnaggingm_w3]', '$_POST[msnaggingm_l4]', '$_POST[msnaggingm_w4]', '$_POST[msnam_note]',
					'$_POST[msp_grdl1]', '$_POST[msp_clsl1]', '$_POST[msp_shol1]', '$_POST[msp_medl1]', '$_POST[msp_lonl1]',
					'$_POST[msp_grdw1]', '$_POST[msp_clsw1]', '$_POST[msp_show1]', '$_POST[msp_medw1]', '$_POST[msp_lonw1]',
					'$_POST[msp_grdl2]', '$_POST[msp_clsl2]', '$_POST[msp_shol2]', '$_POST[msp_medl2]', '$_POST[msp_lonl2]',
					'$_POST[msp_grdw2]', '$_POST[msp_clsw2]', '$_POST[msp_show2]', '$_POST[msp_medw2]', '$_POST[msp_lonw2]',
					'$_POST[msnap_note]',
					'$_POST[msnaggingb_l1]', '$_POST[msnaggingb_w1]', '$_POST[msnaggingb_l2]', '$_POST[msnaggingb_w2]', '$_POST[msnaggingb_l3]', '$_POST[msnaggingb_w3]', '$_POST[msnaggingb_l4]', '$_POST[msnaggingb_w4]', '$_POST[msnab_note]',
					'$_POST[minstron]', '$_POST[mmullen]', '$_POST[mtru_burst]', '$_POST[mtru_burst2]', '$_POST[mburs_note]',
					'$_POST[mthick1]', '$_POST[mthick2]', '$_POST[mthick3]', '$_POST[mthickav]', '$_POST[mthick_note]',
					'$_POST[mstretch_l1]', '$_POST[mstretch_w1]', '$_POST[mstretch_l2]', '$_POST[mstretch_w2]', '$_POST[mstretch_l3]', '$_POST[mstretch_w3]', '$_POST[mstretch_l4]', '$_POST[mstretch_w4]',
					'$_POST[mstretch_l5]', '$_POST[mstretch_w5]', '$_POST[mload_stretch]', '$_POST[mstretch_note]',
					'$_POST[mrecover_l1]', '$_POST[mrecover_w1]', '$_POST[mrecover_l2]', '$_POST[mrecover_w2]', '$_POST[mrecover_l3]', '$_POST[mrecover_w3]', '$_POST[mrecover_l4]', '$_POST[mrecover_w4]',
					'$_POST[mrecover_l5]', '$_POST[mrecover_w5]',
					'$_POST[mrecover_l11]', '$_POST[mrecover_w11]', '$_POST[mrecover_l21]', '$_POST[mrecover_w21]', '$_POST[mrecover_l31]', '$_POST[mrecover_w31]', '$_POST[mrecover_l41]', '$_POST[mrecover_w41]', '$_POST[mrecover_l51]', '$_POST[mrecover_w51]',
					'$_POST[mrecover_note]',
					'$_POST[mgrowth_l1]', '$_POST[mgrowth_w1]', '$_POST[mgrowth_l2]', '$_POST[mgrowth_w2]',
					'$_POST[mrec_growth_l1]', '$_POST[mrec_growth_w1]', '$_POST[mrec_growth_l2]', '$_POST[mrec_growth_w2]',
					'$_POST[mgrowth_note]',
					'$_POST[mapper_ch1]', '$_POST[mapper_ch2]', '$_POST[mapper_ch3]',
					'$_POST[mapper_cc1]', '$_POST[mapper_cc2]', '$_POST[mapper_cc3]',
					'$_POST[mapper_st]', '$_POST[mapper_st2]', '$_POST[mapper_st3]',
					'$_POST[mapper_pf1]', '$_POST[mapper_pf2]', '$_POST[mapper_pf3]',
					'$_POST[mapper_pb1]', '$_POST[mapper_pb2]', '$_POST[mapper_pb3]',
					'$_POST[mapper_acetate]', '$_POST[mapper_cotton]', '$_POST[mapper_nylon]', '$_POST[mapper_poly]', '$_POST[mapper_acrylic]', '$_POST[mapper_wool]',
					'$_POST[mapper_note]',
					'$_POST[mfibre_transfer]', '$_POST[mfibre_grade]', '$_POST[mfibre_note]',
					'$_POST[modour]', '$_POST[modour_note]',
					GETDATE()
				)
			");
			...
			echo "<script>swal({
				...
				if (result.value) {
					window.location.href='TestingNew-$nodemand'; 
				}
			});</script>";
		}
	} else if ($_POST['physical_save'] == "save") {
		$sqlPHY = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tq_test (
				id_nokk,
				flamability, fla_note,
				fc_cott, fc_poly, fc_elastane,
				fc_cott1, fc_poly1, fc_elastane1,
				std_fc_cott1, std_fc_poly1, std_fc_elastane1,
				fibercontent, fiber_note,
				fc_wpi, fc_cpi, fc_note,
				f_weight, fwe_note,
				f_width, fwi_note,
				bow, skew, bas_note,
				h_shrinkage_temp, h_shrinkage_l1, h_shrinkage_w1, h_shrinkage_grd, h_shrinkage_app, h_shrinkage_note,
				ss_temp, ss_washes3, ss_washes10, ss_washes15, ss_cmt,
				shrinkage_l1, shrinkage_l2, shrinkage_l3, shrinkage_l4, shrinkage_l5, shrinkage_l6,
				shrinkage_w1, shrinkage_w2, shrinkage_w3, shrinkage_w4, shrinkage_w5, shrinkage_w6,
				spirality1, spirality2, spirality3, spirality4, spirality5, spirality6,
				ss_linedry, ss_tumbledry, sns_note,
				apperss_ch1, apperss_ch2, apperss_ch3, apperss_ch4,
				apperss_cc1, apperss_cc2, apperss_cc3, apperss_cc4,
				apperss_st,
				apperss_pf1, apperss_pf2, apperss_pf3, apperss_pf4,
				apperss_pb1, apperss_pb2, apperss_pb3, apperss_pb4,
				apperss_sf1, apperss_sf2, apperss_sf3, apperss_sf4,
				apperss_sb1, apperss_sb2, apperss_sb3, apperss_sb4,
				apperss_note,
				pm_f1, pm_b1, pm_f2, pm_b2, pm_f3, pm_b3, pm_f4, pm_b4, pm_f5, pm_b5, pillm_note,
				pl_f1, pl_b1, pl_f2, pl_b2, pl_f3, pl_b3, pl_f4, pl_b4, pl_f5, pl_b5, pilll_note,
				pb_f1, pb_b1, pb_f2, pb_b2, pb_f3, pb_b3, pb_f4, pb_b4, pb_f5, pb_b5, pillb_note,
				prt_f1, prt_b1, prt_f2, prt_b2, prt_f3, prt_b3, prt_f4, prt_b4, prt_f5, prt_b5, pillr_note,
				abration, abr_note,
				sm_l1, sm_w1, sm_l2, sm_w2, sm_l3, sm_w3, sm_l4, sm_w4, snam_note,
				sp_grdl1, sp_clsl1, sp_shol1, sp_medl1, sp_lonl1,
				sp_grdw1, sp_clsw1, sp_show1, sp_medw1, sp_lonw1,
				sp_grdl2, sp_clsl2, sp_shol2, sp_medl2, sp_lonl2,
				sp_grdw2, sp_clsw2, sp_show2, sp_medw2, sp_lonw2,
				snap_note,
				sb_l1, sb_w1, sb_l2, sb_w2, sb_l3, sb_w3, sb_l4, sb_w4, snab_note,
				bs_instron, bs_mullen, bs_tru, bs_tru2, burs_note,
				thick1, thick2, thick3, thickav, thick_note,
				stretch_l1, stretch_w1, stretch_l2, stretch_w2, stretch_l3, stretch_w3, stretch_l4, stretch_w4,
				stretch_l5, stretch_w5, load_stretch, stretch_note,
				recover_l1, recover_w1, recover_l2, recover_w2, recover_l3, recover_w3, recover_l4, recover_w4, recover_l5, recover_w5,
				recover_l11, recover_w11, recover_l21, recover_w21, recover_l31, recover_w31, recover_l41, recover_w41, recover_l51, recover_w51,
				recover_note,
				growth_l1, growth_w1, growth_l2, growth_w2,
				rec_growth_l1, rec_growth_w1, rec_growth_l2, rec_growth_w2,
				growth_note,
				apper_ch1, apper_ch2, apper_ch3,
				apper_cc1, apper_cc2, apper_cc3,
				apper_st, apper_st2, apper_st3,
				apper_pf1, apper_pf2, apper_pf3,
				apper_pb1, apper_pb2, apper_pb3,
				apper_acetate, apper_cotton, apper_nylon, apper_poly, apper_acrylic, apper_wool,
				apper_note,
				fibre_transfer, fibre_grade, fibre_note,
				odour, odour_note,
				curling, curling_note,
				nedle, nedle_note,
				stat_fla, stat_fib, stat_fc,
				stat_fwss, stat_fwss2, stat_fwss3,
				stat_bsk, stat_pm, stat_pl, stat_pb, stat_prt, stat_abr, stat_sm, stat_sp, stat_sb,
				stat_bs, stat_bs2, stat_bs3,
				stat_th, stat_sr, stat_gr, stat_ap, stat_hs, stat_ff,
				stat_odour, stat_curling, stat_nedle,
				wick_l1, wick_w1, wick_l2, wick_w2, wick_l3, wick_w3, wick_note,
				absor_f1, absor_f2, absor_f3, absor_b1, absor_b2, absor_b3, absor_note,
				dry1, dry2, dry3, dry_note,
			...
			VALUES (
				'$rcek[id]',
				'$_POST[flamability]', '$_POST[fla_note]',
				'$_POST[fc_cott]', '$_POST[fc_poly]', '$_POST[fc_ela]',
				'$_POST[fc_cott1]', '$_POST[fc_poly1]', '$_POST[fc_ela1]',
				'$_POST[std_fc_cott1]', '$_POST[std_fc_poly1]', '$_POST[std_fc_ela1]',
				'$_POST[fibercontent]', '$_POST[fiber_note]',
				'$_POST[wpi]', '$_POST[cpi]', '$_POST[fc_note]',
				'$_POST[fabric_weight]', '$_POST[fwe_note]',
				'$_POST[fabric_width]', '$_POST[fwi_note]',
				'$_POST[bow]', '$_POST[skew]', '$_POST[bas_note]',
				'$_POST[h_shrinkage_temp]', '$_POST[h_shrinkage_l1]', '$_POST[h_shrinkage_w1]', '$_POST[h_shrinkage_grd]', '$_POST[h_shrinkage_app]', '$_POST[h_shrinkage_note]',
				'$_POST[ss_temp]', '$_POST[ss_washes3]', '$_POST[ss_washes10]', '$_POST[ss_washes15]', '$_POST[ss_cmt]',
				'$_POST[shrinkage_len1]', '$_POST[shrinkage_len2]', '$_POST[shrinkage_len3]', '$_POST[shrinkage_len4]', '$_POST[shrinkage_len5]', '$_POST[shrinkage_len6]',
				'$_POST[shrinkage_wid1]', '$_POST[shrinkage_wid2]', '$_POST[shrinkage_wid3]', '$_POST[shrinkage_wid4]', '$_POST[shrinkage_wid5]', '$_POST[shrinkage_wid6]',
				'$_POST[spirality1]', '$_POST[spirality2]', '$_POST[spirality3]', '$_POST[spirality4]', '$_POST[spirality5]', '$_POST[spirality6]',
				'$_POST[ss_linedry]', '$_POST[ss_tumbledry]', '$_POST[sns_note]',
				'$_POST[apperss_ch1]', '$_POST[apperss_ch2]', '$_POST[apperss_ch3]', '$_POST[apperss_ch4]',
				'$_POST[apperss_cc1]', '$_POST[apperss_cc2]', '$_POST[apperss_cc3]', '$_POST[apperss_cc4]',
				'$_POST[apperss_st]',
				'$_POST[apperss_pf1]', '$_POST[apperss_pf2]', '$_POST[apperss_pf3]', '$_POST[apperss_pf4]',
				'$_POST[apperss_pb1]', '$_POST[apperss_pb2]', '$_POST[apperss_pb3]', '$_POST[apperss_pb4]',
				'$_POST[apperss_sf1]', '$_POST[apperss_sf2]', '$_POST[apperss_sf3]', '$_POST[apperss_sf4]',
				'$_POST[apperss_sb1]', '$_POST[apperss_sb2]', '$_POST[apperss_sb3]', '$_POST[apperss_sb4]',
				'$_POST[apperss_note]',
				'$_POST[pillingm_f1]', '$_POST[pillingm_b1]', '$_POST[pillingm_f2]', '$_POST[pillingm_b2]', '$_POST[pillingm_f3]', '$_POST[pillingm_b3]', '$_POST[pillingm_f4]', '$_POST[pillingm_b4]', '$_POST[pillingm_f5]', '$_POST[pillingm_b5]', '$_POST[pillm_note]',
				'$_POST[pillingl_f1]', '$_POST[pillingl_b1]', '$_POST[pillingl_f2]', '$_POST[pillingl_b2]', '$_POST[pillingl_f3]', '$_POST[pillingl_b3]', '$_POST[pillingl_f4]', '$_POST[pillingl_b4]', '$_POST[pillingl_f5]', '$_POST[pillingl_b5]', '$_POST[pilll_note]',
				'$_POST[pillingb_f1]', '$_POST[pillingb_b1]', '$_POST[pillingb_f2]', '$_POST[pillingb_b2]', '$_POST[pillingb_f3]', '$_POST[pillingb_b3]', '$_POST[pillingb_f4]', '$_POST[pillingb_b4]', '$_POST[pillingb_f5]', '$_POST[pillingb_b5]', '$_POST[pillb_note]',
				'$_POST[pillingrt_f1]', '$_POST[pillingrt_b1]', '$_POST[pillingrt_f2]', '$_POST[pillingrt_b2]', '$_POST[pillingrt_f3]', '$_POST[pillingrt_b3]', '$_POST[pillingrt_f4]', '$_POST[pillingrt_b4]', '$_POST[pillingrt_f5]', '$_POST[pillingrt_b5]', '$_POST[pillr_note]',
				'$_POST[abr]', '$_POST[abr_note]',
				'$_POST[snaggingm_l1]', '$_POST[snaggingm_w1]', '$_POST[snaggingm_l2]', '$_POST[snaggingm_w2]', '$_POST[snaggingm_l3]', '$_POST[snaggingm_w3]', '$_POST[snaggingm_l4]', '$_POST[snaggingm_w4]', '$_POST[snam_note]',
				'$_POST[sp_grdl1]', '$_POST[sp_clsl1]', '$_POST[sp_shol1]', '$_POST[sp_medl1]', '$_POST[sp_lonl1]',
				'$_POST[sp_grdw1]', '$_POST[sp_clsw1]', '$_POST[sp_show1]', '$_POST[sp_medw1]', '$_POST[sp_lonw1]',
				'$_POST[sp_grdl2]', '$_POST[sp_clsl2]', '$_POST[sp_shol2]', '$_POST[sp_medl2]', '$_POST[sp_lonl2]',
				'$_POST[sp_grdw2]', '$_POST[sp_clsw2]', '$_POST[sp_show2]', '$_POST[sp_medw2]', '$_POST[sp_lonw2]',
				'$_POST[snap_note]',
				'$_POST[snaggingb_l1]', '$_POST[snaggingb_w1]', '$_POST[snaggingb_l2]', '$_POST[snaggingb_w2]', '$_POST[snaggingb_l3]', '$_POST[snaggingb_w3]', '$_POST[snaggingb_l4]', '$_POST[snaggingb_w4]', '$_POST[snab_note]',
				'$_POST[instron]', '$_POST[mullen]', '$_POST[tru_burst]', '$_POST[tru_burst2]', '$_POST[burs_note]',
				'$_POST[thick1]', '$_POST[thick2]', '$_POST[thick3]', '$_POST[thickav]', '$_POST[thick_note]',
				'$_POST[stretch_l1]', '$_POST[stretch_w1]', '$_POST[stretch_l2]', '$_POST[stretch_w2]', '$_POST[stretch_l3]', '$_POST[stretch_w3]', '$_POST[stretch_l4]', '$_POST[stretch_w4]',
				'$_POST[stretch_l5]', '$_POST[stretch_w5]', '$_POST[load_stretch]', '$_POST[stretch_note]',
				'$_POST[recover_l1]', '$_POST[recover_w1]', '$_POST[recover_l2]', '$_POST[recover_w2]', '$_POST[recover_l3]', '$_POST[recover_w3]', '$_POST[recover_l4]', '$_POST[recover_w4]', '$_POST[recover_l5]', '$_POST[recover_w5]',
				'$_POST[recover_l11]', '$_POST[recover_w11]', '$_POST[recover_l21]', '$_POST[recover_w21]', '$_POST[recover_l31]', '$_POST[recover_w31]', '$_POST[recover_l41]', '$_POST[recover_w41]', '$_POST[recover_l51]', '$_POST[recover_w51]',
				'$_POST[recover_note]',
				'$_POST[growth_l1]', '$_POST[growth_w1]', '$_POST[growth_l2]', '$_POST[growth_w2]',
				'$_POST[rec_growth_l1]', '$_POST[rec_growth_w1]', '$_POST[rec_growth_l2]', '$_POST[rec_growth_w2]',
				'$_POST[growth_note]',
				'$_POST[apper_ch1]', '$_POST[apper_ch2]', '$_POST[apper_ch3]',
				'$_POST[apper_cc1]', '$_POST[apper_cc2]', '$_POST[apper_cc3]',
				'$_POST[apper_st]', '$_POST[apper_st2]', '$_POST[apper_st3]',
				'$_POST[apper_pf1]', '$_POST[apper_pf2]', '$_POST[apper_pf3]',
				'$_POST[apper_pb1]', '$_POST[apper_pb2]', '$_POST[apper_pb3]',
				'$_POST[apper_acetate]', '$_POST[apper_cotton]', '$_POST[apper_nylon]', '$_POST[apper_poly]', '$_POST[apper_acrylic]', '$_POST[apper_wool]',
				'$_POST[apper_note]',
				'$_POST[fibre_transfer]', '$_POST[fibre_grade]', '$_POST[fibre_note]',
				'$_POST[odour]', '$_POST[odour_note]',
				'$_POST[curling]', '$_POST[curling_note]',
				'$_POST[nedle]', '$_POST[nedle_note]',
				'$_POST[stat_fla]', '$_POST[stat_fib]', '$_POST[stat_fc]',
				'$_POST[stat_fwss]', '$_POST[stat_fwss2]', '$_POST[stat_fwss3]',
				'$_POST[stat_bsk]', '$_POST[stat_pm]', '$_POST[stat_pl]', '$_POST[stat_pb]', '$_POST[stat_prt]', '$_POST[stat_abr]', '$_POST[stat_sm]', '$_POST[stat_sp]', '$_POST[stat_sb]',
				'$_POST[stat_bs]', '$_POST[stat_bs2]', '$_POST[stat_bs3]',
				'$_POST[stat_th]', '$_POST[stat_sr]', '$_POST[stat_gr]', '$_POST[stat_ap]', '$_POST[stat_hs]', '$_POST[stat_ff]',
				'$_POST[stat_odour]', '$_POST[stat_curling]', '$_POST[stat_nedle]',
				'$_POST[wick_l1]', '$_POST[wick_w1]', '$_POST[wick_l2]', '$_POST[wick_w2]', '$_POST[wick_l3]', '$_POST[wick_w3]', '$_POST[wick_note]',
				'$_POST[absor_f1]', '$_POST[absor_f2]', '$_POST[absor_f3]', '$_POST[absor_b1]', '$_POST[absor_b2]', '$_POST[absor_b3]', '$_POST[absor_note]',
				'$_POST[dry1]', '$_POST[dry2]', '$_POST[dry3]', '$_POST[dry_note]',
				'$_POST[repp1]', '$_POST[repp3]', '$_POST[repp3]', '$_POST[repp4]', '$_POST[repp_note]',
	...
	if ($_POST['colorfastness_save'] == "save" and $cek1 > 0) {
		...
		if ($sqlCLR) {
			...
			$sqlCLRDI = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tq_disptest (
					id_nokk,
					...
					dlight_rating1, dlight_rating2, dlight_note,
					dlight_pers_colorchange, dlight_pers_note,
					dsaliva_staining, dsaliva_note,
					dbleeding, dbleeding_note,
					dchlorin, dnchlorin1, dnchlorin2, dchlorin_note,
					ddye_tf_sstaining, ddye_tf_cstaining,
					ddye_tf_acetate, ddye_tf_cotton, ddye_tf_nylon, ddye_tf_poly, ddye_tf_acrylic, ddye_tf_wool,
					ddye_tf_note,
					tgl_buat, tgl_update
				)
				VALUES (
					...
					'$_POST[dlight_rating1]', '$_POST[dlight_rating2]', '$_POST[dlight_note]',
					'$_POST[dlight_pers_colorchange]', '$_POST[dlight_pers_note]',
					...
					'$_POST[ddye_tf_sstaining]', '$_POST[ddye_tf_cstaining]',
					'$_POST[ddye_tf_acetate]', '$_POST[ddye_tf_cotton]', '$_POST[ddye_tf_nylon]', '$_POST[ddye_tf_poly]', '$_POST[ddye_tf_acrylic]', '$_POST[ddye_tf_wool]',
					'$_POST[ddye_tf_note]',
					GETDATE(), GETDATE()
				)
			");
			$sqlCLRM = sqlsrv_query($con_db_qc_sqlsrv, " UPDATE db_qc.tbl_tq_marginal SET
			...
			$sqlCLRMI = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tq_marginal (
					id_nokk,
					...
					mlight_rating1, mlight_rating2, mlight_note,
					mlight_pers_colorchange, mlight_pers_note,
					msaliva_staining, msaliva_note,
					mbleeding, mbleeding_note,
					mchlorin, mnchlorin1, mnchlorin2, mchlorin_note,
					mdye_tf_sstaining, mdye_tf_cstaining,
					mdye_tf_acetate, mdye_tf_cotton, mdye_tf_nylon, mdye_tf_poly, mdye_tf_acrylic, mdye_tf_wool,
					mdye_tf_note,
					tgl_update
				)
				VALUES (
					...
					'$_POST[mlight_rating1]', '$_POST[mlight_rating2]', '$_POST[mlight_note]',
					'$_POST[mlight_pers_colorchange]', '$_POST[mlight_pers_note]',
					...
					'$_POST[mdye_tf_sstaining]', '$_POST[mdye_tf_cstaining]',
					'$_POST[mdye_tf_acetate]', '$_POST[mdye_tf_cotton]', '$_POST[mdye_tf_nylon]', '$_POST[mdye_tf_poly]', '$_POST[mdye_tf_acrylic]', '$_POST[mdye_tf_wool]',
					'$_POST[mdye_tf_note]',
					GETDATE()
				)
			");
		...
		$sqlCLR = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tq_test (
				id_nokk,
				...
				light_rating1, light_rating2, light_note,
				light_pers_colorchange, light_pers_note,
				saliva_staining, saliva_note,
				bleeding, bleeding_note,
				chlorin, nchlorin1, nchlorin2, chlorin_note,
				dye_tf_sstaining, dye_tf_cstaining,
				dye_tf_acetate, dye_tf_cotton, dye_tf_nylon, dye_tf_poly, dye_tf_acrylic, dye_tf_wool,
				dye_tf_note,
			...
			VALUES (
				...
				'$_POST[light_rating1]', '$_POST[light_rating2]', '$_POST[light_note]',
				'$_POST[light_pers_colorchange]', '$_POST[light_pers_note]',
				...
				'$_POST[dye_tf_sstaining]', '$_POST[dye_tf_cstaining]',
				'$_POST[dye_tf_acetate]', '$_POST[dye_tf_cotton]', '$_POST[dye_tf_nylon]', '$_POST[dye_tf_poly]', '$_POST[dye_tf_acrylic]', '$_POST[dye_tf_wool]',
				'$_POST[dye_tf_note]',
	...
	if ($_POST['functional_save'] == "save" and $cek1 > 0) {
		...
		if ($sqlFPH) {
			...
			$sqlFPHDI = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tq_disptest (
					id_nokk,
					dwick_l1, dwick_w1, dwick_l2, dwick_w2, dwick_l3, dwick_w3, dwick_l4, dwick_w4, dwick_note,
					dabsor_f1, dabsor_f2, dabsor_f3, dabsor_b1, dabsor_b2, dabsor_b3, dabsor_note,
					ddry1, ddry2, ddry3, ddryaf1, ddryaf2, ddryaf3, ddry_note,
					drepp1, drepp2, drepp3, drepp4, drepp_note,
					dph, dph_note,
					dsoil, dsoil_note,
				...
				VALUES (
					'$rcek[id]',
					'$_POST[dwick_l1]', '$_POST[dwick_w1]', '$_POST[dwick_l2]', '$_POST[dwick_w2]', '$_POST[dwick_l3]', '$_POST[dwick_w3]', '$_POST[dwick_l4]', '$_POST[dwick_w4]', '$_POST[dwick_note]',
					'$_POST[dabsor_f1]', '$_POST[dabsor_f2]', '$_POST[dabsor_f3]', '$_POST[dabsor_b1]', '$_POST[dabsor_b2]', '$_POST[dabsor_b3]', '$_POST[dabsor_note]',
					'$_POST[ddry1]', '$_POST[ddry2]', '$_POST[ddry3]', '$_POST[ddryaf1]', '$_POST[ddryaf2]', '$_POST[ddryaf3]', '$_POST[ddry_note]',
					'$_POST[drepp1]', '$_POST[drepp2]', '$_POST[drepp3]', '$_POST[drepp4]', '$_POST[drepp_note]',
			...
			$sqlFPHM = sqlsrv_query($con_db_qc_sqlsrv, " UPDATE db_qc.tbl_tq_marginal SET
			...
			$sqlFPHMI = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tq_marginal (
					id_nokk,
					mwick_l1, mwick_w1, mwick_l2, mwick_w2, mwick_l3, mwick_w3, mwick_l4, mwick_w4, mwick_note,
					mabsor_f1, mabsor_f2, mabsor_f3, mabsor_b1, mabsor_b2, mabsor_b3, mabsor_note,
					mdry1, mdry2, mdry3, mdryaf1, mdryaf2, mdryaf3, mdry_note,
					mrepp1, mrepp2, mrepp3, mrepp4, mrepp_note,
					mph, mph_note,
					msoil, msoil_note,
				...
				VALUES (
					'$rcek[id]',
					'$_POST[mwick_l1]', '$_POST[mwick_w1]', '$_POST[mwick_l2]', '$_POST[mwick_w2]', '$_POST[mwick_l3]', '$_POST[mwick_w3]', '$_POST[mwick_l4]', '$_POST[mwick_w4]', '$_POST[mwick_note]',
					'$_POST[mabsor_f1]', '$_POST[mabsor_f2]', '$_POST[mabsor_f3]', '$_POST[mabsor_b1]', '$_POST[mabsor_b2]', '$_POST[mabsor_b3]', '$_POST[mabsor_note]',
					'$_POST[mdry1]', '$_POST[mdry2]', '$_POST[mdry3]', '$_POST[mdryaf1]', '$_POST[mdryaf2]', '$_POST[mdryaf3]', '$_POST[mdry_note]',
					'$_POST[mrepp1]', '$_POST[mrepp2]', '$_POST[mrepp3]', '$_POST[mrepp4]', '$_POST[mrepp_note]',
		...
		$sqlFPH = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tq_test (
				id_nokk,
				wick_l1, wick_w1, wick_l2, wick_w2, wick_l3, wick_w3, wick_l4, wick_w4, wick_note,
				absor_f1, absor_f2, absor_f3, absor_b1, absor_b2, absor_b3, absor_note,
			...
			VALUES (
				'$rcek[id]',
				'$_POST[wick_l1]', '$_POST[wick_w1]', '$_POST[wick_l2]', '$_POST[wick_w2]', '$_POST[wick_l3]', '$_POST[wick_w3]', '$_POST[wick_l4]', '$_POST[wick_w4]', '$_POST[wick_note]',
				'$_POST[absor_f1]', '$_POST[absor_f2]', '$_POST[absor_f3]', '$_POST[absor_b1]', '$_POST[absor_b2]', '$_POST[absor_b3]', '$_POST[absor_note]',
				'$_POST[dry1]', '$_POST[dry2]', '$_POST[dry3]', '$_POST[dryaf1]', '$_POST[dryaf2]', '$_POST[dryaf3]', '$_POST[dry_note]',
				'$_POST[repp1]', '$_POST[repp2]', '$_POST[repp3]', '$_POST[repp4]', '$_POST[repp_note]',
				'$_POST[ph]', '$_POST[ph_note]',
				'$_POST[soil]', '$_POST[soil_note]',
