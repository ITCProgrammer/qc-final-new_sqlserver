...
<table width="100%">
  ...
  <tr>
        <td colspan="3" align="center"><strong><font size="+1">LAPORAN HASIL PACKING SHIFT <?php echo $Shift; ?></font><br />
  ...
  <tr>
    <td align="left" valign="top"><table width="100%" border="1" class="table-list1">
      <thead>
        ...
        <tr align="center">
          <td colspan="4"><font size="-2"><strong>QTY</strong></font></td>
        </tr>
        <tr align="center">
      ...
      <tbody>
        <?php 
            ...
            WHERE DATEADD(second, DATEDIFF(second, 0, TRY_CAST(jam_update AS TIME)), TRY_CAST(tgl_update AS DATETIME)) between '$start_date' and '$stop_date' AND inspektor='PACKING A' $nomc $grp AND dept='PACKING' GROUP BY operator,no_mc");
                while($row=sqlsrv_fetch_array($qry1, SQLSRV_FETCH_ASSOC)){
                    //QTY KECIL
                    $qryKecil=sqlsrv_query($con_db_qc_sqlsrv,"SELECT SUM(jml_netto) as roll, SUM(bruto) AS bruto, SUM(netto) AS netto, SUM(panjang) AS panjang FROM db_qc.tbl_lap_inspeksi
                    ...
                    //QTY BESAR
                    $qryBesar=sqlsrv_query($con_db_qc_sqlsrv,"SELECT SUM(jml_netto) as roll, SUM(bruto) AS bruto, SUM(netto) AS netto, SUM(panjang) AS panjang FROM db_qc.tbl_lap_inspeksi
        ...
        <tr>
          ...
          <td align="center"><input type=text name=nama placeholder="Ketik disini" style="width: 98%;"></td>
        </tr>
        <?php
				...
				// QTY LOSS
				$qryLossA=sqlsrv_query($con_db_qc_sqlsrv,"SELECT
					SUM(qty_loss) as kg_loss
				from
					db_qc.tbl_lap_inspeksi
				where
					DATEADD(second, DATEDIFF(second, 0, TRY_CAST(jam_update AS TIME)), TRY_CAST(tgl_update AS DATETIME)) between '$start_date' and '$stop_date'
					and inspektor = 'PACKING A'
					and dept = 'PACKING'");
                $rowLossA=sqlsrv_fetch_array($qryLossA, SQLSRV_FETCH_ASSOC);
				?>
        <tr>
    ...
    <td align="left" valign="top"><table width="100%" border="1" class="table-list1">
      <thead>
        ...
        <tr align="center">
          <td colspan="2">Supervisor</td>
          <td colspan="4" bgcolor="#D7D5D5">Ali Mulhakim</td>
        </tr>
        <tr align="center">
          <td rowspan="3"><font size="-2"><strong>OPERATOR</strong></font></td>
          <td rowspan="3"><font size="-2"><strong>NO MESIN</strong></font></td>
...
//            if($_GET['shift']!="ALL"){$shft=" AND `shift`='$_GET[shift]' "; }else{$shft=" ";}
            ...
            WHERE DATEADD(second, DATEDIFF(second, 0, TRY_CAST(jam_update AS TIME)), TRY_CAST(tgl_update AS DATETIME)) between '$start_date' and '$stop_date' AND inspektor='PACKING B' $nomc $grp AND dept='PACKING' GROUP BY operator,no_mc");
                while($row=sqlsrv_fetch_array($qry1, SQLSRV_FETCH_ASSOC)){
                    ...
                    $rowBesar=sqlsrv_fetch_array($qryBesar, SQLSRV_FETCH_ASSOC);
					// QTY LOSS
					$qryLossB=sqlsrv_query($con_db_qc_sqlsrv,"SELECT
						SUM(qty_loss) as kg_loss
					from
						db_qc.tbl_lap_inspeksi
					where
						...
						and dept = 'PACKING'");
                    $rowLossB=sqlsrv_fetch_array($qryLossB, SQLSRV_FETCH_ASSOC);
					$sqlop="SELECT x.LONGDESCRIPTION FROM DB2ADMIN.INITIALS x
							WHERE CODE ='".$row['operator']."'"; 
                    $stmt1=db2_exec($conn1,$sqlop, array('cursor'=>DB2_SCROLLABLE));
                    $rowop = db2_fetch_assoc($stmt1);
        ...
        <tr>
          ...
          <td align="center"><?php echo $rowKecil['netto']+$rowBesar['netto'];?></td>
          <td align="center"><?php echo $rowKecil['panjang']+$rowBesar['panjang'];?></td>
          <td align="center"><input type=text name=nama1 placeholder="Ketik disini" style="width: 98%;"></td>
        </tr>
        <?php
                $t_rollK=$t_rollK+$rowKecil['roll'];
                $t_brutoK=$t_brutoK+$rowKecil['bruto'];
    ...
    <td align="left" valign="top"><table width="100%" border="1" class="table-list1">
      <thead>
        ...
        <tr align="center">
          <td colspan="2">Supervisor</td>
          <td colspan="4" bgcolor="#D7D5D5">Heriyanto</td>
        </tr>
        <tr align="center">
          <td rowspan="3"><font size="-2"><strong>OPERATOR</strong></font></td>
          <td rowspan="3"><font size="-2"><strong>NO MESIN</strong></font></td>
...
//            if($_GET['shift']!="ALL"){$shft=" AND `shift`='$_GET[shift]' "; }else{$shft=" ";}
            ...
            WHERE DATEADD(second, DATEDIFF(second, 0, TRY_CAST(jam_update AS TIME)), TRY_CAST(tgl_update AS DATETIME)) between '$start_date' and '$stop_date' AND inspektor='PACKING C' $nomc $grp AND dept='PACKING' GROUP BY operator,no_mc");
                while($row=sqlsrv_fetch_array($qry1, SQLSRV_FETCH_ASSOC)){
                    ...
                    $rowBesar=sqlsrv_fetch_array($qryBesar, SQLSRV_FETCH_ASSOC);
					// QTY LOSS
					$qryLossC=sqlsrv_query($con_db_qc_sqlsrv,"SELECT
						SUM(qty_loss) as kg_loss
					from
						db_qc.tbl_lap_inspeksi
					where
						...
						and dept = 'PACKING'");
                    $rowLossC=sqlsrv_fetch_array($qryLossC, SQLSRV_FETCH_ASSOC);
					$sqlop="SELECT x.LONGDESCRIPTION FROM DB2ADMIN.INITIALS x
							WHERE CODE ='".$row['operator']."'"; 
                    $stmt1=db2_exec($conn1,$sqlop, array('cursor'=>DB2_SCROLLABLE));
                    $rowop = db2_fetch_assoc($stmt1);
        ...
        <tr>
          ...
          <td align="center"><?php echo $rowKecil['netto']+$rowBesar['netto'];?></td>
          <td align="center"><?php echo $rowKecil['panjang']+$rowBesar['panjang'];?></td>
          <td align="center"><input type=text name=nama2 placeholder="Ketik disini" style="width: 98%;"></td>
        </tr>
        <?php
                $t_rollK=$t_rollK+$rowKecil['roll'];
                $t_brutoK=$t_brutoK+$rowKecil['bruto'];
