...
$sqlCek = sqlsrv_query($con_db_qc_sqlsrv, "SELECT TOP 1 * FROM db_qc.tbl_memo_qcf WHERE nodemand='$nodemand' ORDER BY id DESC");
...
function no_urut()
{
	...
	$sql = sqlsrv_query($con_db_qc_sqlsrv, "SELECT TOP 1 no_ncp FROM db_qc.tbl_ncp_qcf WHERE SUBSTRING(no_ncp,1,10) LIKE '" . $format . "%' ORDER BY no_ncp DESC");
...
if ($_POST['save'] == "Simpan") {
	...
	$sqlData = sqlsrv_query(
		$con_db_qc_sqlsrv,
		"INSERT INTO db_qc.tbl_memo_qcf (
			nokk, nodemand, langganan, buyer, no_order, no_hanger, no_item,
			po, po_rajut, supp_rajut, jenis_kain, lebar, gramasi, lot,
			rol, warna, no_warna, masalah, berat, dept,
			peninjau_awal, ket, tempat, tgl_buat, tgl_update
		) VALUES (
			'$_POST[nokk]', '$_POST[nodemand]', '$_POST[pelanggan]', '$_POST[buyer]',
			'$_POST[no_order]', '$_POST[no_hanger]', '$_POST[no_item]', '$po',
			'$_POST[po_rajut]', '$_POST[supp_rajut]', '$jns', '$lebar',
			'$grms', '$lot', '$rol', '$warna', '$nowarna', '$kt1',
			'$_POST[berat]', '$_POST[dept]', '$_POST[peninjau_awal]', '$ket',
			'$_POST[tempat]', GETDATE(), GETDATE()
		)"
	);
