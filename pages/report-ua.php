...
$sqlCek=sqlsrv_query($con_db_qc_sqlsrv,"SELECT TOP 1 * FROM db_qc.tbl_tq_nokk WHERE CASE
        WHEN nodemand_new <> '' THEN nodemand_new
        ELSE nodemand
    END =  '$nodemand' 
		OR 
		CASE
...
TRIM(A.SUBCODE02) AS SUBCODE02, TRIM(A.SUBCODE03) AS SUBCODE03, TRIM(C.LONGDESCRIPTION) AS NO_ITEM,TRIM(C.PO_NUMBER) AS PO_NUMBER, TRIM(C.STYL) AS STYL FROM PRODUCTIONDEMAND A 
LEFT JOIN 
...
LEFT JOIN 
    ...
    END AS STYL,
    SUM(SALESORDERLINE.BASEPRIMARYQUANTITY) AS QTY_ORDER,SUM(SALESORDERLINE.BASESECONDARYQUANTITY) AS QTY_PANJANG_ORDER, 
	...
	SALESORDER.EXTERNALREFERENCE,SALESORDERLINE.EXTERNALREFERENCE,SALESORDER.INTERNALREFERENCE,SALESORDERLINE.INTERNALREFERENCE, SALESORDERDELIVERY.DELIVERYDATE, ORDERITEMORDERPARTNERLINK.LONGDESCRIPTION, ORDERPARTNERBRAND.LONGDESCRIPTION) C
...
$rowdb2 = db2_fetch_assoc($stmt);
    function autono_report($notest)
    {
	include "koneksi.php";
        date_default_timezone_set('Asia/Jakarta');
        $thn= date("Y");
		$today= date("ymd");
        $sqlnorptua = sqlsrv_query($con_db_qc_sqlsrv,"SELECT TOP 1 no_rptua FROM db_qc.tbl_tq_nokk WHERE SUBSTRING(no_rptua,4,6) like '%".$today."%' ORDER BY no_rptua DESC") or die(sqlsrv_errors());
        $dt=sqlsrv_has_rows($sqlnorptua) ? 1 : 0;
        if ($dt>0) {
    		$rd=sqlsrv_fetch_array($sqlnorptua);
            $dt=$rd['no_rptua'];
            $strd=substr($dt, 10, 2);
            $Urutd = (int)$strd;
        } else {
...
<div class="box box-info">
	 	...
	 	<div class="col-md-6">
                  ...
                  <label for="no_po" class="col-sm-3 control-label">No Demand</label>
                  	<div class="col-sm-4">
						<div class="input-group">	  
							...
							onchange="window.location='ReportUA-'+this.value" value="<?php if($_GET['nodemand']!=""){echo $_GET['nodemand'];}else{echo $nodemand_post;}?>" placeholder="No Demand" required <?php if($_SESSION['lvl_id']=="TQ"){echo "readonly";}?>>
						</div>	  
		  			</div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-6">
                    ...
                    value="<?php if($nodemand!="" AND ($rcek['no_rptua']=="" OR $rcek['no_rptua']==NULL)){echo autono_report($rcek['no_test']); }else if($nodemand!="" AND $rcek['no_rptua']!=""){echo 'INDO-'.$rcek['season'].$uaproduct.$rcek['no_hanger'].'-'.$rcek['warna'].'-'.$rcek['no_rptua'];}else{echo $ReportUA;} ?>">
                ...
                <div class="col-sm-4">					  
					<div class="input-group date">
						<div class="input-group-addon"> <i class="fa fa-calendar"></i> </div>
							<input name="date_in" type="text" class="form-control pull-right" id="datepicker" placeholder="0000-00-00" value="<?php if($rcek['date_in']!="" OR $rcek['date_in']!=NULL){echo ($rcek['date_in'] ? date_format($rcek['date_in'], 'Y-m-d') : '');}else{echo $DateIn;} ?>" required autocomplete="off"/>
					</div>
				</div>
            </div>
            <div class="form-group">
				...
				<div class="col-sm-8">
					...
					value="<?php if($rcek['style']!="" OR $rcek['style']!=NULL){echo $rcek['style'];}else if($rowdb2['STYL']!=''){echo $rowdb2['STYL'];}else{echo $Style;} ?>" placeholder="Style">
				</div>				   
            </div> 
            <div class="form-group">
	...
	?>	 
   	<div class="box-footer"> 
		...
		<?php }else if($rcekNew['status']!="" AND strpos($rcek['no_item'],'DKI18021') !== false){?>
			<a href="pages/cetak/cetak_report_ua_dki18021.php?idkk=<?php echo $rcek['id'];?>&noitem=<?php echo $rcek['no_item'];?>&nohanger=<?php echo $rcek['no_hanger'];?>" target="_blank" class="btn btn-primary"><i class="fa fa-print"></i> Print Report Under Armour</a> 
		<?php }else if($rcekNew['status']!="" AND strpos($rcek['no_item'],'DPQ22133') !== false){?>
			<a href="pages/cetak/cetak_report_ua_dpq22133.php?idkk=<?php echo $rcek['id'];?>&noitem=<?php echo $rcek['no_item'];?>&nohanger=<?php echo $rcek['no_hanger'];?>" target="_blank" class="btn btn-primary"><i class="fa fa-print"></i> Print Report Under Armour</a> 
		<?php } else if($rcekNew['status']!="xxx" AND strpos($rcek['no_item'],'DKI21012') !== false){?>
			<a href="pages/cetak/cetak_report_ua_dki21012.php?idkk=<?php echo $rcek['id'];?>&noitem=<?php echo $rcek['no_item'];?>&nohanger=<?php echo $rcek['no_hanger'];?>" target="_blank" class="btn btn-primary"><i class="fa fa-print"></i> Print Report Under Armour</a> 
		<?php // cetak_report_ua_dpq22133
			 // cetak_report_ua_dki21012
		}		
		?>
   	</div>
    <!-- /.box-footer -->
...
$sqlPHYMI=sqlsrv_query($con_db_qc_sqlsrv,"INSERT INTO tbl_tq_marginal (
	id_nokk,
	mflamability,
	mfla_note,
	mfc_cott,
	...
	GETDATE()
		...
		if (result.value) {
			...
			window.location.href='ReportUA-$nodemand'; 
		}
		});</script>";
	}
}else if($_POST['physical_save']=="save"){
	$sql = "INSERT INTO db_qc.tbl_tq_test (
		id_nokk, flamability, fla_note, fc_cott, fc_poly, fc_elastane, fc_cott1, fc_poly1, fc_elastane1, 
		fibercontent, fiber_note, fc_wpi, fc_cpi, fc_note, f_weight, fwe_note, f_width, fwi_note, 
		bow, skew, bas_note, h_shrinkage_l1, h_shrinkage_w1, h_shrinkage_grd, h_shrinkage_app, h_shrinkage_note, 
		...
		spirality1, spirality2, spirality3, spirality4, spirality5, spirality6, sns_note, 
		apperss_ch1, apperss_ch2, apperss_cc1, apperss_cc2, apperss_st, apperss_pf1, apperss_pf2, apperss_pb1, apperss_pb2, apperss_note, 
		...
		bs_instron, bs_mullen, bs_tru, burs_note, 
		...
		recover_l1, recover_w1, recover_l2, recover_w2, recover_l3, recover_w3, recover_l4, recover_w4, recover_l5, recover_w5, recover_note, 
		...
		stat_abr, stat_sm, stat_sp, stat_sb, stat_bs, stat_bs2, stat_bs3, stat_th, stat_sr, stat_gr, stat_ap, stat_hs, stat_ff, 
		...
		'$rcek[id]', '$_POST[flamability]', '$_POST[fla_note]', '$_POST[fc_cott]', '$_POST[fc_poly]', '$_POST[fc_ela]', '$_POST[fc_cott1]', '$_POST[fc_poly1]', '$_POST[fc_ela1]', 
		'$_POST[fibercontent]', '$_POST[fiber_note]', '$_POST[wpi]', '$_POST[cpi]', '$_POST[fc_note]', '$_POST[fabric_weight]', '$_POST[fwe_note]', '$_POST[fabric_width]', '$_POST[fwi_note]', 
		'$_POST[bow]', '$_POST[skew]', '$_POST[bas_note]', '$_POST[h_shrinkage_l1]', '$_POST[h_shrinkage_w1]', '$_POST[h_shrinkage_grd]', '$_POST[h_shrinkage_app]', '$_POST[h_shrinkage_note]', 
		...
		'$_POST[spirality1]', '$_POST[spirality2]', '$_POST[spirality3]', '$_POST[spirality4]', '$_POST[spirality5]', '$_POST[spirality6]', '$_POST[sns_note]', 
		'$_POST[apperss_ch1]', '$_POST[apperss_ch2]', '$_POST[apperss_cc1]', '$_POST[apperss_cc2]', '$_POST[apperss_st]', '$_POST[apperss_pf1]', '$_POST[apperss_pf2]', '$_POST[apperss_pb1]', '$_POST[apperss_pb2]', '$_POST[apperss_note]', 
		...
		'$_POST[instron]', '$_POST[mullen]', '$_POST[tru_burst]', '$_POST[burs_note]', 
		...
		'$_POST[recover_l1]', '$_POST[recover_w1]', '$_POST[recover_l2]', '$_POST[recover_w2]', '$_POST[recover_l3]', '$_POST[recover_w3]', '$_POST[recover_l4]', '$_POST[recover_w4]', '$_POST[recover_l5]', '$_POST[recover_w5]', '$_POST[recover_note]', 
		...
		'$_POST[stat_abr]', '$_POST[stat_sm]', '$_POST[stat_sp]', '$_POST[stat_sb]', '$_POST[stat_bs]', '$_POST[stat_bs2]', '$_POST[stat_bs3]', '$_POST[stat_th]', '$_POST[stat_sr]', '$_POST[stat_gr]', '$_POST[stat_ap]', '$_POST[stat_hs]', '$_POST[stat_ff]', 
	...
	$sqlPHY=sqlsrv_query($con_db_qc_sqlsrv,$sql);
...
$sql = "INSERT INTO db_qc.tbl_tq_disptest (
	id_nokk, dwash_temp, dwash_colorchange, dwash_acetate, dwash_cotton, dwash_nylon, dwash_poly, dwash_acrylic, dwash_wool, dwash_staining, dwash_note,
	...
	'$rcek[id]', '$_POST[dwash_temp]', '$_POST[dwash_colorchange]', '$_POST[dwash_acetate]', '$_POST[dwash_cotton]', '$_POST[dwash_nylon]', '$_POST[dwash_poly]', '$_POST[dwash_acrylic]', '$_POST[dwash_wool]', '$_POST[dwash_staining]', '$_POST[dwash_note]',
...
$sqlCLRDI=sqlsrv_query($con_db_qc_sqlsrv,$sql);
...
if($_POST['save1']=="save"){
	...
	$sqlKK=sqlsrv_query($con_db_qc_sqlsrv,"UPDATE db_qc.tbl_tq_nokk SET
	date_out='$date_out',
	style='$sty',
...
if($_POST['simpan_season']=="Simpan"){
	...
	$sqlData1=sqslsrv_query($con_db_qc_sqlsrv,"INSERT INTO db_qc.tbl_season_validity (nama) VALUES ('$nama')");
	if($sqlData1){	
	echo "<script>swal({
