...
$sql = " SELECT TOP (1) *
    ...
    WHERE (CASE WHEN ISNULL(nodemand_new,'') <> '' THEN nodemand_new ELSE nodemand END) = ?
    ORDER BY id DESC
";

$stmt = sqlsrv_query($con_db_qc_sqlsrv, $sql, [$nodemand]);
if ($stmt === false) {
...
<div class="box box-info">
   	<div class="box-header with-border">
    	<h3 class="box-title">Testing First Lot</h3>
  	...
  	<div class="box-body"> 
	 	<div class="col-md-6">
                  ...
                  <label for="no_po" class="col-sm-3 control-label">No Demand</label>
                  	<div class="col-sm-4">
						<div class="input-group">	  
							...
							onchange="window.location='TestingNewFL-'+this.value" value="<?php echo $_GET['nodemand'];?>" placeholder="No Demand" required <?php if($_SESSION['lvl_id']=="TQ"){echo "readonly";}?>>
						</div>	  
					</div>
			...
			<div class="form-group">
					...
					<div class="col-sm-5">
						<input name="kk_legacy" type="text" class="form-control" id="kk_legacy" placeholder="No KK Legacy"
						value="<?php if($cek>0){echo $rcek['kk_legacy'];} ?>" readonly="readonly" >
					</div>
					<?php if($cek>0){?><a href="#" class="btn btn-info btn-xs posisi_kktq" id="<?php echo $rcek['kk_legacy']; ?>">Posisi Terakhir</a><?php }?>
                </div>
			<div class="form-group">
				...
				<script>
					function handleChange() {
						...
						window.location = "TestingNewNoTesFL-" + modifiedValue;
					}
				</script>
	
					<input name="no_test" type="text" class="form-control" id="no_test" placeholder="No Report FL" autocomplete="off" 
					onchange="handleChange()" value="<?php if($cek>0){echo $rcek['no_report_fl'];}else{echo $_GET['no_report_fl'];} ?>" >
				</div>				   
			</div>
	        <div class="form-group">
	  	...
	  	<div class="col-md-6">
			...
			<div class="form-group">
					...
					<div class="col-sm-5">
						<input name="lot_legacy" type="text" class="form-control" id="lot_legacy" placeholder="No Lot Legacy"
						value="<?php if($cek>0){echo $rcek['no_lot_legacy'];} ?>" readonly="readonly" >
					</div>
                </div>
		  	<div class="form-group">
                <label for="No Report FL" class="col-sm-3 control-label" >No Report FL</label>
                <div class="col-sm-8">				
                    	<input name="no_report_fl" type="text" class="form-control" required="required"  
						value="<?php if($cek>0){echo $rcek['no_report_fl'];} ?>" readonly="readonly"
						>										
                </div>				   
            </div>
            <div class="form-group">
          		<label for="target_kirim" class="col-sm-3 control-label">Target Kirim</label>
          		<div class="col-sm-8">
			  		<input name="target_kirim" type="text" class="form-control"   required="required" value="<?=
						(!empty($rcek) && !empty($rcek['target_kirim']))
						? (($rcek['target_kirim'] instanceof DateTime)
							? $rcek['target_kirim']->format('Y-m-d')
							: date('Y-m-d', strtotime($rcek['target_kirim'])))
						: ''
					?>" 			
	  				readonly="readonly" >
		  		</div>
...
if($sqlCmt=== false){die(print_r(sqlsrv_errors(), true));}
$rcekcmt = ($sqlCmt !== false) ? sqlsrv_fetch_array($sqlCmt, SQLSRV_FETCH_ASSOC) : null;
?>
          					...
          					<div class="col-sm-4">
              					...
              					<option selected="selected" value="">Pilih</option>
									<?php
										...
										$result = sqlsrv_query($con_db_qc_sqlsrv, $sql, [$rcek['no_test']]);
										if ($result === false) {
											die(print_r(sqlsrv_errors(), true));
										}

										while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
											$detail = explode(",", (string)($row['physical'] ?? ''));
								...
								<div class="col-sm-3">
									...
									<option value="">Pilih</option>	  
										<?php
											...
											while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
												$detail = explode(",", (string)($row['colorfastness'] ?? ''));
												foreach ($detail as $value) {
													$value = trim($value);
													if ($value === '') continue;
													echo '<option value="'.htmlspecialchars($value, ENT_QUOTES).'">'
							...
							<div class="col-sm-3">
								...
								<option value="">Pilih</option>	  
									<?php
										...
										while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
											$detail = explode(",", (string)($row['functional'] ?? ''));
											foreach ($detail as $value) {
												$value = trim($value);
												if ($value === '') continue;
												echo '<option value="'.htmlspecialchars($value, ENT_QUOTES).'">'
...
<section class="invoice">
      ...
      <div class="row invoice-info">
		...
		<?php } ?>
		<?php if($rcek1['dye_tf_cstaining']!="" or $rcek1['dye_tf_acetate']!="" or $rcek1['dye_tf_cotton']!="" or $rcek1['	']!="" or $rcek1['dye_tf_poly']!="" or $rcek1['dye_tf_acrylic']!="" or $rcek1['dye_tf_wool']!="" or $rcek1['dye_tf_sstaining']!=""){
      		?>	
      	<tr>
		...
		if ( strlen($rcek1['sco_acid_original']) >2 
		...
		?>
		<tr>
			<th>Sweat Conceal Original Acid</th>
		    <td><?=$sco_acid_original[0] ?></td>
		    <td colspan="2"></td>
			<td></td>
		    <td colspan="2"></td>
          	<td>&nbsp;</td>
		...
		<tr>
			<th>Sweat Conceal Afterwash Acid</th>
		    <td><?=$sca_acid_original[0]?></td>
		    <td colspan="2"></td>
			<td></td>
		    <td colspan="2"></td>
          	<td>&nbsp;</td>
		...
		<tr>
			<th>Sweat Conceal Original Alkaline	</th>
		    <td><?=$sco_alkaline_afterwash[0]?></td>
		    <td colspan="2"></td>
			<td></td>
		    <td colspan="2"></td>
          	<td>&nbsp;</td>
		...
		<tr>
			<th>Sweat Conceal Afterwash Alkaline</th>
		    <td><?=$sca_alkaline_afterwash[0]?></td>
		    <td colspan="2"></td>
			<td></td>
		    <td colspan="2"></td>
          	<td>&nbsp;</td>
...
<?php
	if($_POST['physical_save']=="save" and $cek1>0){
		...
		$sql = "UPDATE db_qc.tbl_tq_test_fl SET
			[flamability]=?,
			[fla_note]=?,
			[fc_cott]=?,
			[fc_poly]=?,
		...
		if($sqlPHY){
			$sqlPHYD = sqlsrv_query($con_db_qc_sqlsrv,"UPDATE db_qc.tbl_tq_disptest_fl SET 
			...
			$sqlPHYDI = sqlsrv_query($con_db_qc_sqlsrv,"INSERT INTO db_qc.tbl_tq_disptest_fl
				...
				[dstretch_l1],[dstretch_w1],[dstretch_l2],[dstretch_w2],[dstretch_l3],[dstretch_w3],[dstretch_l4],[dstretch_w4],[dstretch_l5],[dstretch_w5],
				[dload_stretch],[dstretch_note],
				[drecover_l1],[drecover_w1],[drecover_l2],[drecover_w2],[drecover_l3],[drecover_w3],[drecover_l4],[drecover_w4],[drecover_l5],[drecover_w5],
				[drecover_l11],[drecover_w11],[drecover_l21],[drecover_w21],[drecover_l31],[drecover_w31],[drecover_l41],[drecover_w41],[drecover_l51],[drecover_w51],
				[drecover_note],
			...
			VALUES
				...
				'$_POST[dstretch_l1]','$_POST[dstretch_w1]','$_POST[dstretch_l2]','$_POST[dstretch_w2]','$_POST[dstretch_l3]','$_POST[dstretch_w3]','$_POST[dstretch_l4]','$_POST[dstretch_w4]','$_POST[dstretch_l5]','$_POST[dstretch_w5]',
				'$_POST[dload_stretch]','$_POST[dstretch_note]',
				...
				'$_POST[drecover_l11]','$_POST[drecover_w11]','$_POST[drecover_l21]','$_POST[drecover_w21]','$_POST[drecover_l31]','$_POST[drecover_w31]','$_POST[drecover_l41]','$_POST[drecover_w41]','$_POST[drecover_l51]','$_POST[drecover_w51]',
				'$_POST[drecover_note]',
				...
				$sqlPHYM = sqlsrv_query($con_db_qc_sqlsrv, $sql, $params);
				...
				$sql = "INSERT INTO db_qc.tbl_tq_marginal_fl (
					...
					[mstretch_l1],[mstretch_w1],[mstretch_l2],[mstretch_w2],[mstretch_l3],[mstretch_w3],[mstretch_l4],[mstretch_w4],[mstretch_l5],[mstretch_w5],
					[mload_stretch],[mstretch_note],
				...
				echo "<script>swal({
					...
					if (result.value) {
						window.location.href='TestingNewFL-$nodemand'; 
					}
				});</script>";
			}
		}else if($_POST['physical_save']=="save"){
		
		$sql = "INSERT INTO db_qc.tbl_tq_test_fl (
			[id_nokk],
			...
			[stretch_l1],[stretch_w1],[stretch_l2],[stretch_w2],[stretch_l3],[stretch_w3],[stretch_l4],[stretch_w4],[stretch_l5],[stretch_w5],
			[load_stretch],[stretch_note],
			...
			[stat_bsk],
			[stat_pm],[stat_pl],[stat_pb],[stat_prt],
			[stat_abr],[stat_sm],[stat_sp],[stat_sb],
...
if($_POST['colorfastness_save']=="save" and $cek1>0){
	$sql = "UPDATE [db_qc].[tbl_tq_test_fl] SET
		...
		[sca_acid_original]=?,
		[sca_acid_status]=?,
		[sco_alkaline_afterwash]=?,
		[sco_alkaline_status]=?,
		[sca_alkaline_afterwash]=?,
		[sca_alkaline_status]=?,
		[sc_note]=?
	WHERE [id_nokk]=?";
	...
	$params = [
		...
		$_POST['water_acrylic'],
		$_POST['water_wool'],
		$_POST['water_staining'],
		$_POST['water_note'],
		$_POST['acid_colorchange'],
		$_POST['acid_acetate'],
		$_POST['acid_cotton'],
		$_POST['acid_nylon'],
		$_POST['acid_poly'],
		...
		$_POST['acid_staining'],
		$_POST['acid_note'],
		$_POST['alkaline_colorchange'],
		$_POST['alkaline_acetate'],
		$_POST['alkaline_cotton'],
		$_POST['alkaline_nylon'],
		$_POST['alkaline_poly'],
		$_POST['alkaline_acrylic'],
		$_POST['alkaline_wool'],
		$_POST['alkaline_staining'],
		$_POST['alkaline_note'],
		$_POST['crock_len1'],
		$_POST['crock_wid1'],
		$_POST['crock_len2'],
		$_POST['crock_wid2'],
		$_POST['crock_note'],
		$_POST['phenolic_colorchange'],
		$_POST['phenolic_note'],
		$_POST['cm_printing_colorchange'],
		$_POST['cm_printing_staining'],
		$_POST['cm_printing_note'],
		$_POST['cm_dye_temp'],
		$_POST['cm_dye_colorchange'],
		$_POST['cm_dye_stainingface'],
		$_POST['cm_dye_stainingback'],
		$_POST['cm_dye_note'],
		$_POST['light_rating1'],
		$_POST['light_rating2'],
		$_POST['light_note'],
		$_POST['light_pers_colorchange'],
		$_POST['light_pers_note'],
		$_POST['saliva_staining'],
		$_POST['saliva_note'],
		$_POST['bleeding'],
		$_POST['bleeding_note'],
		$_POST['chlorin'],
		...
		$_POST['nchlorin2'],
		$_POST['chlorin_note'],
		$_POST['dye_tf_sstaining'],
		$_POST['dye_tf_cstaining'],
		$_POST['dye_tf_acetate'],
		$_POST['dye_tf_cotton'],
		$_POST['dye_tf_nylon'],
		$_POST['dye_tf_poly'],
		$_POST['dye_tf_acrylic'],
		$_POST['dye_tf_wool'],
		$_POST['dye_tf_note'],
		$_POST['stat_wf'],
		$_POST['stat_wtr'],
		...
		$_POST['stat_dye'],
		$sco_original,
		$_POST['sco_acid_status'],
		$sca_acid,
		$_POST['sca_acid_status'],
		$sco_alkaline_afterwash,
		$_POST['sco_alkaline_status'],
		$sca_alkaline_afterwash,
		$_POST['sca_alkaline_status'],
		$_POST['sc_note'],
		$rcek['id']
	];
	...
	if($sqlCLR){
		...
		$sql = "UPDATE [db_qc].[tbl_tq_disptest_fl] SET
			[dwash_temp]=?,
			[dwash_colorchange]=?,
			[dwash_acetate]=?,
			[dwash_cotton]=?,
		...
		$params = [
			...
			$_POST['dwash_acrylic'],
			$_POST['dwash_wool'],
			$_POST['dwash_staining'],
			$_POST['dwash_note'],
			$_POST['dwater_colorchange'],
			$_POST['dwater_acetate'],
			$_POST['dwater_cotton'],
			$_POST['dwater_nylon'],
			$_POST['dwater_poly'],
			$_POST['dwater_acrylic'],
			$_POST['dwater_wool'],
			$_POST['dwater_staining'],
			$_POST['dwater_note'],
			$_POST['dacid_colorchange'],
			$_POST['dacid_acetate'],
			$_POST['dacid_cotton'],
			$_POST['dacid_nylon'],
			$_POST['dacid_poly'],
			$_POST['dacid_acrylic'],
			$_POST['dacid_wool'],
			$_POST['dacid_staining'],
			$_POST['dacid_note'],
			$_POST['dalkaline_colorchange'],
			$_POST['dalkaline_acetate'],
			$_POST['dalkaline_cotton'],
			$_POST['dalkaline_nylon'],
			$_POST['dalkaline_poly'],
			$_POST['dalkaline_acrylic'],
			$_POST['dalkaline_wool'],
			$_POST['dalkaline_staining'],
			$_POST['dalkaline_note'],
			$_POST['dcrock_len1'],
			$_POST['dcrock_wid1'],
			$_POST['dcrock_len2'],
			$_POST['dcrock_wid2'],
			$_POST['dcrock_note'],
			$_POST['dphenolic_colorchange'],
			$_POST['dphenolic_note'],
			$_POST['dcm_printing_colorchange'],
			$_POST['dcm_printing_staining'],
			$_POST['dcm_printing_note'],
			$_POST['dcm_dye_temp'],
			$_POST['dcm_dye_colorchange'],
			$_POST['dcm_dye_stainingface'],
			$_POST['dcm_dye_stainingback'],
			$_POST['dcm_dye_note'],
			$_POST['dlight_rating1'],
			$_POST['dlight_rating2'],
			$_POST['dlight_note'],
			$_POST['dlight_pers_colorchange'],
			$_POST['dlight_pers_note'],
			$_POST['dsaliva_staining'],
			$_POST['dsaliva_note'],
			$_POST['dbleeding'],
			$_POST['dbleeding_note'],
			$_POST['dchlorin'],
			$_POST['dnchlorin1'],
			$_POST['dnchlorin2'],
			$_POST['dchlorin_note'],
			$_POST['ddye_tf_sstaining'],
			$_POST['ddye_tf_cstaining'],
			$_POST['ddye_tf_acetate'],
			$_POST['ddye_tf_cotton'],
			$_POST['ddye_tf_nylon'],
			$_POST['ddye_tf_poly'],
			$_POST['ddye_tf_acrylic'],
			$_POST['ddye_tf_wool'],
			$_POST['ddye_tf_note'],
			$rcek['id']
		];
		...
		$sqlCLRDI = sqlsrv_query($con_db_qc_sqlsrv, $sql, $params);
		// if($sqlCLRDI === false){ die(print_r(sqlsrv_errors(), true)); }
		...
		$sql = "UPDATE [db_qc].[tbl_tq_marginal_fl] SET
			...
			[mphenolic_note]=?,
			[mcm_printing_colorchange]=?,
			[mcm_printing_staining]=?,
			[mcm_printing_note]=?,
			[mcm_dye_temp]=?,
			[mcm_dye_colorchange]=?,
			[mcm_dye_stainingface]=?,
			[mcm_dye_stainingback]=?,
			[mcm_dye_note]=?,
			[mlight_rating1]=?,
			[mlight_rating2]=?,
			[mlight_note]=?,
		...
		$params = [
			...
			$_POST['mwash_acrylic'],
			$_POST['mwash_wool'],
			$_POST['mwash_staining'],
			$_POST['mwash_note'],
			$_POST['mwater_colorchange'],
			$_POST['mwater_acetate'],
			$_POST['mwater_cotton'],
			$_POST['mwater_nylon'],
			$_POST['mwater_poly'],
			$_POST['mwater_acrylic'],
			$_POST['mwater_wool'],
			$_POST['mwater_staining'],
			$_POST['mwater_note'],
			$_POST['macid_colorchange'],
			$_POST['macid_acetate'],
			$_POST['macid_cotton'],
			$_POST['macid_nylon'],
			$_POST['macid_poly'],
			$_POST['macid_acrylic'],
			$_POST['macid_wool'],
			$_POST['macid_staining'],
			$_POST['macid_note'],
			$_POST['malkaline_colorchange'],
			$_POST['malkaline_acetate'],
			$_POST['malkaline_cotton'],
			$_POST['malkaline_nylon'],
			$_POST['malkaline_poly'],
			$_POST['malkaline_acrylic'],
			$_POST['malkaline_wool'],
			$_POST['malkaline_staining'],
			$_POST['malkaline_note'],
			$_POST['mcrock_len1'],
			$_POST['mcrock_wid1'],
			$_POST['mcrock_len2'],
			$_POST['mcrock_wid2'],
			$_POST['mcrock_note'],
			$_POST['mphenolic_colorchange'],
			$_POST['mphenolic_note'],
			$_POST['mcm_printing_colorchange'],
			$_POST['mcm_printing_staining'],
			$_POST['mcm_printing_note'],
			$_POST['mcm_dye_temp'],
			$_POST['mcm_dye_colorchange'],
			$_POST['mcm_dye_stainingface'],
			$_POST['mcm_dye_stainingback'],
			$_POST['mcm_dye_note'],
			$_POST['mlight_rating1'],
			$_POST['mlight_rating2'],
			$_POST['mlight_note'],
			$_POST['mlight_pers_colorchange'],
			$_POST['mlight_pers_note'],
			$_POST['msaliva_staining'],
			$_POST['msaliva_note'],
			$_POST['mbleeding'],
			$_POST['mbleeding_note'],
			$_POST['mchlorin'],
			$_POST['mnchlorin1'],
			$_POST['mnchlorin2'],
			$_POST['mchlorin_note'],
			$_POST['mdye_tf_sstaining'],
			$_POST['mdye_tf_cstaining'],
			$_POST['mdye_tf_acetate'],
			$_POST['mdye_tf_cotton'],
			$_POST['mdye_tf_nylon'],
			$_POST['mdye_tf_poly'],
			$_POST['mdye_tf_acrylic'],
			$_POST['mdye_tf_wool'],
			$_POST['mdye_tf_note'],
			$rcek['id']
		];

		$sqlCLRM = sqlsrv_query($con_db_qc_sqlsrv, $sql, $params);
		if($sqlCLRM === false){ die(print_r(sqlsrv_errors(), true)); }
		...
		$sqlCLRMI = sqlsrv_query($con_db_qc_sqlsrv, $sql, $params);
		// if($sqlCLRMI === false){ die(print_r(sqlsrv_errors(), true)); }
...
if($_POST['functional_save']=="save" and $cek1>0){
	...
	if($sqlFPH){
		...
		$sql = "INSERT INTO [db_qc].[tbl_tq_disptest_fl] (
			[id_nokk],
			[dwick_l1],[dwick_w1],[dwick_l2],[dwick_w2],[dwick_l3],[dwick_w3],[dwick_l4],[dwick_w4],
			[dwick_note],
		...
		$sqlFPHDI = sqlsrv_query($con_db_qc_sqlsrv, $sql, $params);
		// if($sqlFPHDI === false){ die(print_r(sqlsrv_errors(), true)); }
	...
	if($nodemand!="" and $cek==0){
		echo "<script>swal({
			...
			if (result.value) {
				...
				window.location.href='TestingNewFL'; 
		}
	});</script>";
}
?>
