...
if ($_POST['simpan'] == "simpan") {
    $ceksql = sqlsrv_query($con_db_qc_sqlsrv, "SELECT TOP 1 *
        FROM db_qc.tbl_tempel_beda_roll
        WHERE nokk = ?
        AND shift = ?
        AND CAST(tgl_update AS DATE) = CAST(? AS DATE)
        ", [$_GET['nokk'], $_POST['shift'], $today]
    );
    ...
    if ($cek > 0) {
        ...
        $sql1 = sqlsrv_query($con_db_qc_sqlsrv, "UPDATE db_qc.tbl_tempel_beda_roll 
            SET
                nokk                = '$_POST[demand]',
                no_order            = '$order',
        ...
        $sql = sqlsrv_query($con_db_qc_sqlsrv, " INSERT INTO db_qc.tbl_tempel_beda_roll (
            nokk,
            no_order,
            no_item,
            no_hanger,
            ...
            '$_POST[roll_bruto]',
            '$_POST[bruto]',
            '$_POST[roll_dikerjakan]',
            '$_POST[ket_roll_dikerjakan]',
            '$_POST[operator]',
            '$_POST[lebar]',
            '$_POST[gramasi]',
            '$_POST[comment]',
...
$msql = sqlsrv_query($con_db_qc_sqlsrv, "SELECT * FROM db_qc.tbl_tempel_beda_roll WHERE demand='$demand' and shift='$_GET[shift]' 
    AND CAST(tgl_update AS DATE) = CAST('$today' AS DATE)");
$row = sqlsrv_fetch_array($msql, SQLSRV_FETCH_ASSOC);

$csql = sqlsrv_query($con_db_qc_sqlsrv, "SELECT COUNT(*) FROM db_qc.tbl_tempel_beda_roll WHERE demand='$demand' and shift='$_GET[shift]' 
    AND CAST(tgl_update AS DATE) = CAST('$today' AS DATE)");
