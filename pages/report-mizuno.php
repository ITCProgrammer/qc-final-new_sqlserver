...
$NoKKcek=sqlsrv_has_rows($qryNoKK) ? 1 : 0; 
$rNoKK=sqlsrv_fetch_array($qryNoKK);
...
<form class="form-horizontal" action="" method="post" enctype="multipart/form-data" name="form0" id="form0">
 <div class="box box-success" style="width: 98%;">
   <div class="box-header with-border">
    <h3 class="box-title">Report Mizuno</h3>
  ...
  <div class="box-body"> 
                        ...
                        <div class="col-sm-5">
                            <div class="input-group">	  
                                <input name="no_test" type="text" class="form-control" id="no_test" 
                                    onchange="window.location='ReportMizuno-'+this.value" onBlur="window.location='ReportMizuno-'+this.value" value="<?php if($_GET['no_test']!=""){echo $_GET['no_test'];}?>" placeholder="No Test" required >
   ...
   <div class="box-footer"> 
		...
		?>
 	        <a href="pages/cetak/standard_cetak.php?id=<?php echo $rNoKK['id'];?>&category=<?=$ms?>" target="_blank" class="btn btn-success" <?php if($rcek1['status']=="Reject"):?> onclick="return confirm('Summary Test Quality berstatus Reject, apakah ingin tetap mencetak laporan?')" <?php endif; ?>><i class="fa fa-print"></i> Print <?=$ms?></a>
			&nbsp;&nbsp;
				<?php }}?>
				...
				<table id="lookup" class="table table-bordered table-hover table-striped" width="100%">
					...
					<tbody>
						<?php
                                ...
                                $sql = sqlsrv_query($con_db_qc_sqlsrv,"SELECT a.* FROM db_qc.tbl_tq_nokk a INNER JOIN db_qc.tbl_tq_test b ON a.id=b.id_nokk WHERE LENGTH(a.no_test) = 12 and a.nodemand!='' and (a.pelanggan like'%mizuno%' or a.buyer like'%mizuno%') ");
                                $no="1";
