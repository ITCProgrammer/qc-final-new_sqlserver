  ...
  <div class="col-xs-12">
    <div class="box">
      <div class="box-header with-border">
        <h3 class="box-title"> Filter Laporan Jahit Shading</h3>
...
<div class="row">
  <div class="col-xs-12">
    <div class="box">
      <div class="box-header with-border">
        <h3 class="box-title">Data Laporan Jahit Shading</h3><br>
        <?php if($_POST['awal']!="") { ?><b>Periode: <?php echo $_POST['awal']." to ".$_POST['akhir']; ?></b>
        <?php } ?>
        <div class="pull-right">
          <a href="pages/cetak/cetak-reports-jahit-shading.php?awal=<?php echo $_POST['awal']; ?>&akhir=<?php echo $_POST['akhir']; ?>&jam_awal=<?php echo $_POST['jam_awal']; ?>&jam_akhir=<?php echo $_POST['jam_akhir']; ?>&langganan=<?php echo $_POST['langganan']; ?>&no_po=<?php echo $_POST['no_po']; ?>&no_order=<?php echo $_POST['no_order']; ?>&warna=<?php echo $_POST['warna']; ?>&no_item=<?php echo $_POST['no_item']; ?>&no_hanger=<?php echo $_POST['no_hanger']; ?>&shift=<?php echo $Shift; ?>" class="btn btn-primary <?php if($_POST['awal']=="") { echo "disabled"; }?>" target="_blank">Cetak</a> 
        </div>
      </div>
      <div class="box-body">
        <table class="table table-bordered table-hover table-striped nowrap" id="example3" style="width:100%">
          <thead class="bg-blue">
            <tr>
              ...
              <th><div align="center">Reject</div></th>
              <th><div align="center">%</div></th>
              <th><div align="center">Roll Oke</div></th>
              <th><div align="center">Roll Reject</div></th>
          ...
          <?php
            ...
            if($Awal!="" or $PO!="" or $Order!="" or $Shift!=""){
              $qry1=sqlsrv_query($con_db_qc_sqlsrv,"SELECT * FROM db_qc.tbl_lap_jahit_shading WHERE CAST(tgl_update AS DATE) BETWEEN '$Awal' AND '$Akhir' $nopo $noorder $shft ORDER BY id ASC");
            }else{
              $qry1=sqlsrv_query($con_db_qc_sqlsrv,"SELECT * FROM db_qc.tbl_lap_jahit_shading WHERE CAST(tgl_update AS DATE) BETWEEN CAST('$Awal' AS DATE) AND CAST('$Akhir' AS DATE) $nopo $noorder $shft ORDER BY id ASC");
            }
          ...
          ?>
            <tr bgcolor="<?php echo $bgcolor; ?>">
              ...
              <a href="#" class="btn btn-danger btn-xs <?php if($_SESSION['akses']=='biasa' AND ($_SESSION['lvl_id']!='PACKING' OR $_SESSION['lvl_id']!='NCP')){ echo "disabled"; } ?>" onclick="confirm_delete('./HapusDataJahitShading-<?php echo $row1['id'] ?>');"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="Hapus"></i> </a>
              </div></td>
